...
describe("turn loop", () => {
  ...
  it("clears Treat As suit icon effects before round-end Solstice", () => {
    ...
    G.treatedSuitIconsThisTurn = {
      "1": [{ from: "uncivilized", to: ["civilized"] }]
    };

    onTurnEnd(G, { currentPlayer: "1", playOrder: ["1"] } as any);

    expect(G.treatedSuitIconsThisTurn["1"]).toEqual([]);
  ...
  it("moves a resolved action card to discard after play", () => {
    ...
    playCard({ G, ctx }, card);
    expect(G.players["1"].playArea).not.toContain(card);
  ...
  it("keeps a resolved in-play card in play after its play text resolves", () => {
    ...
    expect(G.players["1"].playArea).toContain(card);
  ...
  it("defers after-play hooks and discard until a played card's pending choice resolves", () => {
    ...
    expect(G.players["1"].resources.influence).toBe(3);
    expect(G.players["1"].playArea).not.toContain(card);
    expect(G.players["1"].discard).toContain(card);
  ...
  it("pauses after a triggering effect sentence for a reactive Exhaust before resuming later effects", () => {
    ...
    G.players["1"].playArea = [exhaustCardId];
    G.players["1"].actionsRemaining = 1;
    G.players["1"].actionTokensAvailable = 1;
  ...
  it("matches resource reactive Exhausts by the suited in-play card that produced the resource", () => {
    ...
    G.players["1"].playArea = [sourceCardId, exhaustCardId];
    G.players["1"].exhaustTokensAvailable = 2;
  ...
  it("does not match source-suited reactive Exhausts for resources from a different in-play suit", () => {
    ...
    expect(G.players["1"].resources.knowledge).toBe(1);
    expect(G.players["1"].resources.influence).toBe(0);
    expect(G.players["1"].exhaustTokensAvailable).toBe(1);
  });
  ...
  it("pauses after playing a card for a reactive Exhaust before discarding the played card", () => {
    ...
    expect(G.players["1"].playArea).toContain(playedCard);
    expect(G.players["1"].discard).not.toContain(playedCard);
  ...
  it("pauses after breaking through for a card for a reactive Exhaust before resuming later effects", () => {
    ...
    expect(G.players["1"].hand).toContain(marketCardId);
    expect(G.players["1"].resources.materials).toBe(0);
    expect(G.players["1"].resources.influence).toBe(0);
    expect(G.pendingReactiveExhaustChoice).toMatchObject({
      playerId: "1",
  ...
  it("continues after-Break-through hooks for later gained cards after a hook choice resolves", () => {
    ...
    G.players["1"].hand = [playedCard, "discard_hook_card_a", "discard_hook_card_b"];
    G.players["1"].actionsRemaining = 1;
    G.players["1"].actionTokensAvailable = 1;
    G.players["1"].resources.goods = 0;
    G.players["1"].resources.knowledge = 0;
    ...
    expect(G.players["1"].hand).toEqual(expect.arrayContaining([firstGainedCardId, secondGainedCardId]));
  ...
  it("keeps later resource costs behind a pending discard-card choice", () => {
    ...
    G.players["1"].hand = [playedCard, "discard_a", "discard_b", "keep_card"];
    G.players["1"].actionsRemaining = 1;
    G.players["1"].actionTokensAvailable = 1;
    ...
    expect(G.players["1"].discard).toEqual(["discard_a", "discard_b", playedCard]);
    expect(G.players["1"].resources.knowledge).toBe(0);
    expect(G.players["1"].resources.materials).toBe(1);
  });
  ...
  it("pauses after an opponent takes Unrest for a reactive Exhaust before resuming later effects", () => {
    ...
    G.cardDb[playedCard] = {
      ...
      tags: [],
      effects: [
        { trigger: "on_play", op: "take_unrest", targetPlayerIds: ["2"], count: 1 } as any,
        { trigger: "on_play", op: "gain_resource", resource: "materials", amount: 1 } as any
      ]
    };
    G.cardDb[exhaustCardId] = {
    ...
    expect(G.players["2"].hand).toContain("reactive_unrest_card");
    expect(G.players["1"].resources.materials).toBe(0);
    expect(G.pendingReactiveExhaustChoice).toMatchObject({
      playerId: "1",
      cardIds: [exhaustCardId],
  ...
  it("opens an Unrest reactive window after a paused first take before taking the next Unrest", () => {
    ...
    expect(G.players["1"].hand).toContain("paused_multi_unrest_a");
    expect(G.players["1"].hand).not.toContain("paused_multi_unrest_b");
    expect(G.pendingChoice).toBeDefined();
  ...
  it("keeps a pending Unrest-take continuation parked behind cleanup discard choices", () => {
    ...
    G.pendingCleanupDiscardChoice = { playerId: "1", cardIds: ["discard_option"] };

    const result = continuePendingUnrestTake(G, "1");
    ...
    expect(G.players["1"].hand).not.toContain("parked_unrest");
  ...
  it("keeps a pending Unrest-take continuation parked behind Solstice order choices", () => {
    ...
    G.pendingSolsticeOrderChoice = { playerId: "1", phase: "on_solstice", cardIds: ["first_solstice_card", "second_solstice_card"] };
    ...
    expect(G.players["1"].hand).not.toContain("solstice_order_unrest");
  ...
  it("keeps a pending Unrest-take continuation parked behind paused Solstice continuations", () => {
    ...
    expect(G.players["1"].hand).not.toContain("paused_solstice_unrest");
  ...
  it("pauses after acquiring a card for a reactive Exhaust before resuming later effects", () => {
    ...
    expect(G.players["1"].hand).toContain(acquiredCardId);
    expect(G.players["1"].resources.materials).toBe(0);
    expect(G.pendingReactiveExhaustChoice).toMatchObject({
      playerId: "1",
      cardIds: [exhaustCardId],
  ...
  it("opens direct Market Acquire tucked Unrest reactive windows before market resource windows", () => {
    ...
    G.players["1"].hand = [playedCard];
    G.players["1"].playArea = [unrestExhaustCardId, resourceExhaustCardId];
    G.players["1"].actionsRemaining = 1;
    G.players["1"].actionTokensAvailable = 1;
    G.players["1"].exhaustTokensAvailable = 1;
    G.market = [acquiredCardId];
  ...
  it("pauses after collecting resources from a recalled Region for a reactive Exhaust before resuming later effects", () => {
    ...
    G.players["1"].hand = [playedCard];
    G.players["1"].playArea = [regionCardId, exhaustCardId];
    G.players["1"].actionsRemaining = 1;
    G.players["1"].actionTokensAvailable = 1;
    G.players["1"].exhaustTokensAvailable = 1;
    ...
    expect(G.players["1"].hand).toContain(regionCardId);
    expect(G.players["1"].resources.materials).toBe(2);
    expect(G.players["1"].resources.knowledge).toBe(0);
    expect(G.players["1"].resources.influence).toBe(0);
    expect(G.pendingReactiveExhaustChoice).toMatchObject({
  ...
  it("matches source-suited reactive Exhausts against garrisoned resources recalled with a Region", () => {
    ...
    expect(G.players["1"].hand).toContain(regionCardId);
    expect(G.players["1"].hand).toContain(childCardId);
    expect(G.players["1"].resources.knowledge).toBe(1);
    expect(G.pendingReactiveExhaustChoice).toMatchObject({
      playerId: "1",
      cardIds: [exhaustCardId],
  ...
  it("matches source-suited reactive Exhausts against pending Region choices with garrisoned resources", () => {
    ...
    G.players["1"].hand = [playedCard];
    G.players["1"].playArea = [targetRegionCardId, otherRegionCardId, exhaustCardId];
    G.players["1"].actionsRemaining = 1;
    G.players["1"].actionTokensAvailable = 1;
    G.players["1"].exhaustTokensAvailable = 1;
    G.players["1"].resources.materials = 0;
    ...
    expect(G.players["1"].hand).toContain(targetRegionCardId);
    expect(G.players["1"].hand).toContain(childCardId);
    expect(G.players["1"].resources.knowledge).toBe(1);
    expect(G.pendingReactiveExhaustChoice).toMatchObject({
      playerId: "1",
  ...
  it("pauses after collecting resources from a History-bound card for a reactive Exhaust before resuming later effects", () => {
    ...
    expect(G.players["1"].history).toContain(playedCard);
    expect(G.players["1"].discard).not.toContain(playedCard);
    expect(G.players["1"].resources.materials).toBe(2);
    expect(G.players["1"].resources.knowledge).toBe(0);
    expect(G.players["1"].resources.influence).toBe(0);
  ...
  it("keeps disabled-History resource collection reactive before later self-History text resumes", () => {
    ...
    expect(G.players["1"].history).not.toContain(playedCard);
    expect(G.players["1"].discard).toContain(playedCard);
    expect(G.players["1"].resources.materials).toBe(2);
    expect(G.players["1"].resources.knowledge).toBe(0);
    expect(G.pendingReactiveExhaustChoice).toMatchObject({
  ...
  it("matches source-suited reactive Exhausts against garrisoned resources moved by History text", () => {
    ...
    G.players["1"].playArea = [hostCardId, exhaustCardId];
    G.players["1"].resources.materials = 0;
    G.players["1"].resources.knowledge = 0;
    G.players["1"].resources.influence = 0;
    G.players["1"].exhaustTokensAvailable = 1;
    ...
    const resolved = runEffects({ G, playerId: "1", selfCardId: hostCardId }, [
      { trigger: "on_play", op: "move_self_to_history" } as any,
      { trigger: "on_play", op: "gain_resource", resource: "materials", amount: 1 } as any
    ]);

    expect(resolved).toBe(true);
    expect(G.players["1"].history).toEqual([hostCardId, childCardId]);
    expect(G.players["1"].resources.knowledge).toBe(1);
    expect(G.pendingReactiveExhaustChoice).toMatchObject({
      playerId: "1",
      cardIds: [exhaustCardId],
  ...
  it("pauses after exact Find moves a play-area host to History and collects resources", () => {
    ...
    expect(G.players["1"].playArea).not.toContain(hostCardId);
    expect(G.players["1"].history).toEqual([hostCardId, childCardId]);
    expect(G.players["1"].resources.materials).toBe(2);
    expect(G.players["1"].resources.goods).toBe(1);
    expect(G.players["1"].resources.knowledge).toBe(0);
  ...
  it("pauses after resolving a pending Acquire choice for a reactive Exhaust before resuming later effects", () => {
    ...
    expect(G.players["1"].hand).toContain("reactive_market_b");
    expect(G.players["1"].resources.materials).toBe(0);
    expect(G.pendingReactiveExhaustChoice).toMatchObject({
      playerId: "1",
      cardIds: [exhaustCardId],
  ...
  it("opens a source-suited reactive Exhaust window after collecting resources from a resolved action card", () => {
    ...
    G.players["1"].hand = [card];
    G.players["1"].playArea = [reactiveCard];
    G.players["1"].exhaustTokensAvailable = 1;
    G.players["1"].resources.materials = 0;
    G.players["1"].resources.knowledge = 0;
    G.cardStates = { [card]: { resources: { materials: 2 } } };
  ...
  it("moves garrisoned cards with a resolved action card when it goes to discard", () => {
    ...
    expect(G.players["1"].discard).toEqual([card, garrisonedCard]);
    expect(G.players["1"].resources.materials).toBe(2);
    expect(G.players["1"].resources.knowledge).toBe(1);
    expect(G.cardStates?.[card]).toEqual({ actionTokens: 1 });
  ...
  it("spends an Action token from the State card when playing a normal action", () => {
    ...
    const card = "test_action_archive_survey";
    G.players["1"].hand = [card];
    G.players["1"].actionsRemaining = 3;
    G.players["1"].actionTokensAvailable = 3;
  ...
  it("pauses cleanup turn handoff when draw-up reshuffle creates a pending choice", () => {
    ...
    onTurnEnd(G, { currentPlayer: "1", playOrder: ["1", "2"] } as any, () => 0);
    ...
    resolveChoice({ G, ctx: { currentPlayer: "1", playOrder: ["1", "2"] } as any, random: { Number: () => 0 } }, 0);
  ...
  it("resumes cleanup draw-up after an after-reshuffle reactive Exhaust window", () => {
    ...
    skipReactiveExhaustChoice({ G, ctx: { currentPlayer: "1", playOrder: ["1", "2"] } as any, random: { Number: () => 0 } });
  ...
  it("pauses cleanup turn handoff when a collapse override creates a pending choice", () => {
    ...
    expect(G.pendingChoice).toBeDefined();
    expect(G.pendingTurnEndCleanup).toEqual({ playerId: "1", playOrder: ["1", "2"], stage: "after_draw_up" });
    expect(G.players["1"].resources.knowledge).toBe(0);
    expect(G.log.some((entry) => entry.message === "TurnPhase(turn_handoff): end_turn_complete")).toBe(false);
  ...
  it("Free play cards do not spend Actions and cannot be Free played twice in the same turn", () => {
    ...
    expect(G.players["1"].discard).toContain(card);
    expect(G.players["1"].actionsRemaining).toBe(0);
    expect(G.freePlayedThisTurn?.["1"]).toEqual([card]);
  ...
  it("Free play cards still require printed resource costs", () => {
    ...
    expect(G.players["1"].actionsRemaining).toBe(0);
    expect(G.players["1"].actionTokensAvailable).toBe(0);
    expect(G.freePlayedThisTurn?.["1"]).toBeUndefined();
  ...
  it("free-play effects choose a hand card, resolve its text, and resume source effects", () => {
    const G = createInitialState();
    G.players["1"].hand = ["source_free_play", "free_target"];
    G.players["1"].actionsRemaining = 0;
    G.players["1"].actionTokensAvailable = 0;
    G.players["1"].resources.materials = 1;
    G.players["1"].resources.knowledge = 0;
    ...
    expect(G.players["1"].resources.knowledge).toBe(1);
    expect(G.players["1"].resources.influence).toBe(1);
    expect(G.freePlayedThisTurn?.["1"]).toEqual(expect.arrayContaining(["source_free_play", "free_target"]));
  });

  it("blocks playing cards whose state requirement does not match the visible State card", () => {
    const G = createInitialState();
    G.players["1"].stateArea = ["barbarian_state"];
    ...
    G.players["1"].hand = ["empire_only_action"];
    G.players["1"].actionsRemaining = 1;

    playCard({ G, ctx }, "empire_only_action");

    expect(G.players["1"].hand).toEqual(["empire_only_action"]);
    expect(G.players["1"].playArea).toEqual([]);
    expect(G.players["1"].resources.knowledge).toBe(0);
    expect(G.players["1"].actionsRemaining).toBe(1);
  ...
  it("allows playing cards whose multi-state requirement includes the visible State card", () => {
    ...
    G.players["1"].hand = ["multi_state_action"];
    G.players["1"].actionsRemaining = 1;
    G.players["1"].actionTokensAvailable = 1;
    ...
    expect(G.players["1"].hand).toEqual([]);
    expect(G.players["1"].discard).toContain("multi_state_action");
    expect(G.players["1"].resources.knowledge).toBe(1);
  });
  ...
  it("does not treat discard Unrest as a resolvable default Return Unrest source", () => {
    ...
    G.players["1"].hand = [card];
    G.players["1"].discard = [unrestCard];
    G.players["1"].actionsRemaining = 1;
    G.players["1"].actionTokensAvailable = 1;
    G.unrestPile = [];
    ...
    expect(G.players["1"].hand).toEqual([card]);
    expect(G.players["1"].discard).toEqual([unrestCard]);
  ...
  it("plays default Return Unrest when the only returnable hand card has an imported Unrest suit icon", () => {
    ...
    G.players["1"].hand = [card, unrestCard];
    G.players["1"].discard = [];
    G.players["1"].actionsRemaining = 1;
    G.players["1"].actionTokensAvailable = 1;
    G.unrestPile = [];
  ...
  it("does not play Give text with an invalid fixed recipient", () => {
    ...
    G.players["1"].hand = [card, gift];
    G.players["1"].actionsRemaining = 1;
    G.players["1"].actionTokensAvailable = 1;

    playCard({ G, ctx }, card);

    expect(G.players["1"].hand).toEqual([card, gift]);
    expect(G.players["2"].hand).not.toContain(gift);
    expect(G.players["1"].actionsRemaining).toBe(1);
    expect(G.players["1"].actionTokensAvailable).toBe(1);
    expect(G.log.at(-1)?.message).toBe(`InvalidMove(playCard): no_resolvable_on_play_effects(${card})`);
  });
  ...
  it("does not play targeted Deck Break-through text when the named card is absent", () => {
    ...
    expect(G.players["1"].resources.materials ?? 0).toBe(0);
    expect(G.log.at(-1)?.message).toBe(`InvalidMove(playCard): no_resolvable_on_play_effects(${card})`);
  });
  ...
  it("does not play Gain-card text with a non-market source", () => {
    ...
    expect(G.players["1"].hand).toEqual([card]);
    expect(G.players["1"].discard).not.toContain(card);
    expect(G.players["1"].exile).toEqual(["exiled_civilized"]);
    expect(G.players["1"].actionsRemaining).toBe(1);
    expect(G.players["1"].actionTokensAvailable).toBe(1);
    expect(G.log.at(-1)?.message).toBe(`InvalidMove(playCard): no_resolvable_on_play_effects(${card})`);
  });
  ...
  it("plays Exile-acquire text when only public setup Exile has a matching card", () => {
    ...
    expect(G.players["1"].hand).toContain("setup_civilized");
    expect(G.players["1"].hand).toContain("required_unrest");
    expect(G.players["1"].discard).toContain(card);
    expect(G.log.some((entry) => entry.message === `InvalidMove(playCard): no_resolvable_on_play_effects(${card})`)).toBe(false);
  });
  ...
  it("plays return-Unrest text that targets a nation History replacement zone", () => {
    ...
    G.players["1"].hand = [card];
    G.players["1"].sideAreas = { sunken: [unrestCard] };
    G.players["1"].actionsRemaining = 1;
    G.players["1"].actionTokensAvailable = 1;
    G.unrestPile = [];
  ...
  it("creates a Return Unrest choice from a nation History replacement zone when History is the source", () => {
    ...
    G.players["1"].hand = [card];
    G.players["1"].history = [];
    G.players["1"].sideAreas = { sunken: [unrestCard, secondUnrestCard] };
    G.players["1"].actionsRemaining = 1;
    G.players["1"].actionTokensAvailable = 1;
    G.unrestPile = [];
    ...
    expect(G.players["1"].sideAreas?.sunken).toEqual([unrestCard, secondUnrestCard]);

    resolveReturnUnrestChoice({ G, ctx }, unrestCard);

    expect(G.pendingReturnUnrestChoice).toBeUndefined();
    expect(G.players["1"].sideAreas?.sunken).toEqual([secondUnrestCard]);
    expect(G.unrestPile).toEqual([unrestCard]);
    expect(G.players["1"].discard).toContain(card);
  });
  ...
  it("does not play optional-only zero-count effects", () => {
    ...
    expect(G.players["1"].hand).toEqual([card]);
    expect(G.pendingChoice).toBeUndefined();
    expect(G.players["1"].deck).toEqual(["test_action_foundry_shift"]);
    expect(G.players["1"].actionsRemaining).toBe(1);
    expect(G.players["1"].actionTokensAvailable).toBe(1);
    expect(G.log.at(-1)?.message).toBe(`InvalidMove(playCard): no_resolvable_on_play_effects(${card})`);
  });
  ...
  it("does not play Take-Unrest-only zero-count effects", () => {
    ...
    expect(G.players["1"].hand).toEqual([card]);
    expect(G.players["1"].hand).not.toContain("unrest_1");
  ...
  it("honors selected Progress/Goods substitution when playing a paid action", () => {
    ...
    expect(G.players["1"].resources.goods).toBe(1);
    expect(G.players["1"].resources.materials).toBe(1);
    expect(G.players["1"].hand).toContain("alien_unrest");
    expect(G.players["1"].discard).toContain(card);
  ...
  it("does not trigger spend penalties when returning Progress instead of paying it", () => {
    ...
    expect(G.players["1"].hand).not.toContain("alien_unrest");
    expect(G.players["1"].discard).toContain(card);
  ...
  it("does not trigger spend penalties when stealing Progress instead of paying it", () => {
    ...
    G.cardDb[card] = {
      ...G.cardDb[card],
      effects: [{ trigger: "on_play", op: "steal_resource", fromPlayerId: "2", resource: "knowledge", amount: 1 } as any]
    };
    G.players["1"].hand = [card];
    G.players["1"].resources.knowledge = 0;
    G.players["2"].resources.knowledge = 1;
    G.players["1"].stateArea = ["alien_state"];
    G.players["2"].stateArea = ["alien_state"];
    G.players["1"].actionsRemaining = 1;
    G.players["1"].actionTokensAvailable = 1;
    ...
    G.activeNationRulesets!["2"].stateOverrides = [
      { op: "take_unrest_when_spending_resource", resource: "knowledge", state: "alien" } as any
    ];
    ...
    expect(G.players["1"].hand).not.toContain("alien_unrest_0");
    expect(G.players["2"].hand).not.toContain("alien_unrest_1");
    expect(G.players["1"].discard).toContain(card);
    expect(G.log.some((entry) => entry.message.startsWith("SpentResourcePenalty("))).toBe(false);
  });
  ...
  it("draw-if-able text draws only available deck cards without reshuffling discard", () => {
    ...
    G.players["1"].discard = ["discard_card"];
    G.players["1"].nationDeck = ["nation_card"];
    G.players["1"].actionsRemaining = 1;
    G.players["1"].actionTokensAvailable = 1;
    ...
    expect(G.players["1"].hand).toEqual(["deck_card"]);
    expect(G.players["1"].deck).toEqual([]);
    expect(G.players["1"].discard).toEqual(["discard_card", card]);
    expect(G.players["1"].nationDeck).toEqual(["nation_card"]);
  ...
  it("does not play a recall-only card when the explicit target is not a Region", () => {
    ...
    G.players["1"].hand = [card];
    G.players["1"].playArea = ["not_region"];
    G.players["1"].actionsRemaining = 1;
    G.players["1"].actionTokensAvailable = 1;
    ...
    expect(G.players["1"].hand).toEqual([card]);
    expect(G.players["1"].playArea).toEqual(["not_region"]);
    expect(G.players["1"].discard).not.toContain(card);
    expect(G.log.at(-1)?.message).toBe(`InvalidMove(playCard): no_resolvable_on_play_effects(${card})`);
  });
  ...
  it("auto-resolves unspecified Recall text when the only eligible Region is garrisoned", () => {
    ...
    G.players["1"].hand = [card];
    G.players["1"].playArea = ["non_region_host"];
    G.players["1"].actionsRemaining = 1;
    G.players["1"].actionTokensAvailable = 1;
    ...
    expect(G.players["1"].hand).toContain("garrisoned_region");
  ...
  it("end turn waits for an optional cleanup discard choice before ending the turn", () => {
    ...
    const G = createInitialState();
    G.market = [];
    G.players["1"].hand = ["test_action_archive_survey", "test_action_foundry_shift"];
    ...
    expect(G.players["1"].discard).toContain("test_action_archive_survey");
  });
  ...
  it("resets cleanup token markers before the optional cleanup discard choice", () => {
    ...
    G.players["1"].playArea = ["cleanup_play_card"];
    G.players["1"].powerArea = ["cleanup_power_card"];
    G.players["1"].nationDeck = ["cleanup_nation_card"];
    G.players["1"].developmentArea = ["cleanup_development_card"];
    G.players["1"].discard = ["cleanup_discarded_action"];
    G.players["1"].actionTokensAvailable = 0;
    G.players["1"].exhaustTokensAvailable = 0;
    G.players["1"].progressionTokens = { nationDeck: 1, developmentArea: 1 };
    G.cardStates = {
    ...
    expect(G.players["1"].actionTokensAvailable).toBe(G.players["1"].actionTokensBase);
    expect(G.players["1"].exhaustTokensAvailable).toBe(G.players["1"].exhaustTokensBase);
  });
  ...
  it("precious cards variant prevents voluntary cleanup discard", () => {
    ...
    expect(G.pendingCleanupDiscardChoice).toBeUndefined();
    expect(endTurn).toHaveBeenCalledTimes(1);
    expect(G.players["1"].hand).toEqual(["test_action_archive_survey", "test_action_foundry_shift"]);
  });
  ...
  it("can skip optional cleanup discard", () => {
    ...
    expect(endTurn).toHaveBeenCalledTimes(1);
    expect(G.players["1"].hand).toEqual(["test_action_archive_survey"]);
    expect(G.players["1"].discard).not.toContain("test_action_archive_survey");
  });
  ...
  it("runs solstice only after all players have completed the round", () => {
    const G = createInitialState();
    G.activeNationRulesets!["1"].solsticeOverrides = [{
    ...
    onTurnEnd(G, { currentPlayer: "2" } as any);
  ...
  it("uses the default Solstice state flip when imported rulesets omit state overrides", () => {
    const G = createInitialState();
    G.players["1"].stateArea = ["barbarian_state", "empire_state"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
    G.cardDb.barbarian_state = {
      id: "barbarian_state",
    ...
    expect(G.players["1"].stateArea).toEqual(["empire_state", "barbarian_state"]);
    expect(G.gameover).toBeUndefined();
  });

  it("runs Solstice-triggered effects from play area, Power, and State cards", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["solstice_play_card"];
    G.players["1"].powerArea = ["solstice_power_card"];
    G.players["1"].stateArea = ["solstice_state_card"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
  ...
  it("auto-resolves independent simultaneous Solstice resource gains without an order prompt", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["gain_materials_solstice", "gain_knowledge_solstice"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
  ...
  it("runs end-of-Solstice effects after ordinary Solstice effects", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["solstice_timing_card"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
  ...
  it("does not resolve End-of-Solstice text from a card that left play during Solstice", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["departing_solstice_card"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
    ...
    expect(G.players["1"].history).toEqual(["departing_solstice_card"]);
    expect(G.players["1"].playArea).toEqual([]);
    expect(G.players["1"].resources.knowledge).toBe(0);
  });
  ...
  it("does not resolve End-of-Solstice text from a later source removed by an earlier End-of-Solstice effect", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["end_exile_source", "end_removed_source"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
    ...
    expect(G.players["1"].playArea).toEqual(["end_exile_source"]);
    expect(G.players["1"].exile).toEqual(["end_removed_source"]);
    expect(G.players["1"].resources.knowledge).toBe(0);
  });

  it("does not resolve Solstice text from a later source removed by an earlier ordered Solstice effect", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["solstice_exile_source", "solstice_removed_source"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
    ...
    expect(G.players["1"].playArea).toEqual(["solstice_exile_source"]);
    expect(G.players["1"].exile).toEqual(["solstice_removed_source"]);
    expect(G.players["1"].resources.knowledge).toBe(0);
  });
  ...
  it("activates a configured Go Native state after Reactor-style Solstice removal succeeds", () => {
    ...
    expect(currentStateMatches(G, "1", "native")).toBe(true);
    expect(currentStateMatches(G, "1", "alien")).toBe(false);
  ...
  it("lets a player choose the order of their simultaneous Solstice card effects", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["spend_solstice_card", "gain_solstice_card"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
  ...
  it("lets a player choose Solstice order when one simultaneous effect can Exile a market card", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["exile_solstice_card", "gain_solstice_card"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
    G.market = ["market_civilized", "market_civilized_b"];
    G.cleanupMarketResourcePlaced = { playerId: "2", round: G.round };
  ...
  it("lets a player choose Solstice order when Look can reorder cards before Draw", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["look_solstice_card", "draw_solstice_card"];
    G.players["1"].hand = [];
    G.players["1"].deck = ["draw_first", "draw_second"];
    G.players["2"].hand = [];
    ...
    expect(G.pendingLookOrderChoice).toBeUndefined();
    expect(G.pendingSolsticeContinuation).toBeUndefined();
    expect(G.players["1"].hand).toEqual(["draw_second"]);
    expect(G.players["1"].deck).toEqual(["draw_first"]);
  });

  it("lets a player choose Solstice order when Place on Deck can feed Draw", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["draw_solstice_card", "place_solstice_card"];
    G.players["1"].hand = ["placed_card", "other_hand_card"];
    G.players["1"].deck = ["original_top"];
    G.players["2"].hand = [];
    G.cardDb.draw_solstice_card = {
      id: "draw_solstice_card",
      displayName: "Draw Solstice",
    ...
    expect(G.players["1"].hand).toEqual(["placed_card", "other_hand_card"]);
    ...
    expect(G.pendingSolsticeContinuation?.cardIds).toEqual(["draw_solstice_card"]);
    expect(G.players["1"].hand).toEqual(["placed_card", "other_hand_card"]);
    expect(G.players["1"].deck).toEqual(["original_top"]);

    resolvePlaceOnDeckChoice({ G, ctx }, "placed_card");

    expect(G.pendingPlaceOnDeckChoice).toBeUndefined();
    expect(G.pendingSolsticeContinuation).toBeUndefined();
    expect(G.players["1"].hand).toEqual(["other_hand_card", "placed_card"]);
    expect(G.players["1"].deck).toEqual(["original_top"]);
  });

  it("lets a player choose Solstice order before a Return Unrest choice pauses resolution", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["return_unrest_solstice_card", "gain_solstice_card"];
    G.players["1"].hand = ["hand_unrest", "hand_unrest_b"];
    G.players["2"].hand = [];
  ...
  it("lets a player choose Solstice order before a Return Fame choice pauses resolution", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["return_fame_solstice_card", "gain_solstice_card"];
  ...
  it("lets a player choose Solstice order before a Return Exhaust token choice pauses resolution", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["spent_exhaust_card_a", "spent_exhaust_card_b", "return_exhaust_solstice_card", "gain_solstice_card"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
    G.players["1"].exhaustTokensAvailable = 0;
    G.cardStates = {
  ...
  it("resumes the remaining chosen Solstice order after a card in that order creates a pending choice", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["choice_solstice_card", "gain_solstice_card"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
  ...
  it("resumes remaining chosen Solstice cards with their own card identity after a pending choice", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["choice_solstice_card", "history_solstice_card"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
    G.cardDb.choice_solstice_card = {
      id: "choice_solstice_card",
    ...
    expect(G.players["1"].history).toEqual(["history_solstice_card"]);
    expect(G.players["1"].playArea).toEqual(["choice_solstice_card"]);
    expect(G.round).toBe(2);
  });

  it("uses a configured Solstice state sequence when flipping state", () => {
    const G = createInitialState();
    G.players["1"].stateArea = ["winter_state", "summer_state"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
    ...
    expect(G.players["1"].stateArea).toEqual(["summer_state", "winter_state"]);
  });
  ...
  it("uses a configured Solstice state sequence on a single two-sided State card", () => {
    const G = createInitialState();
    G.players["1"].stateArea = ["seasonal_state_card"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
    ...
    expect(currentStateMatches(G, "1", "winter")).toBe(true);
    expect(currentStateMatches(G, "1", "summer")).toBe(false);

    onTurnEnd(G, { currentPlayer: "2", playOrder: ["1", "2"] } as any);

    expect(G.players["1"].stateArea).toEqual(["seasonal_state_card"]);
    expect(G.cardStates?.seasonal_state_card?.activeState).toBe("summer");
    expect(currentStateMatches(G, "1", "winter")).toBe(false);
    expect(currentStateMatches(G, "1", "summer")).toBe(true);
  });

  it("pauses Solstice when an ordinary Solstice effect creates a pending choice", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["solstice_choice_card"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
  ...
  it("pauses Solstice when an ordinary Solstice effect creates a pending Exile choice", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["solstice_exile_card"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
    G.market = ["market_civilized", "market_civilized_b"];
  ...
  it("pauses Solstice when an ordinary Solstice effect creates a pending Return Unrest choice", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["solstice_return_unrest_card"];
    G.players["1"].hand = ["hand_unrest", "hand_unrest_b"];
  ...
  it("pauses Solstice when an ordinary Solstice effect creates a pending Place on Deck choice", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["solstice_place_card"];
  ...
  it("pauses Solstice when an ordinary Solstice effect creates a pending Give Card choice", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["solstice_give_card"];
    G.players["1"].hand = ["first_card", "second_card"];
    G.players["2"].hand = [];
  ...
  it("pauses Solstice when an ordinary Solstice effect creates a pending Swap choice", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["solstice_swap_card"];
  ...
  it("resumes paused Solstice after a pending Region choice resolves", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["solstice_region_choice", "test_region", "second_region"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
    ...
    expect(G.pendingSolsticeContinuation).toBeUndefined();
    expect(G.pausedSolstice).toBeUndefined();
    expect(G.players["1"].hand).toContain("test_region");
    expect(G.players["1"].resources.knowledge).toBe(1);
    expect(G.round).toBe(2);
  ...
  it("pauses and resumes Solstice when an after-Solstice nation hook creates a pending choice", () => {
    ...
    G.players["1"].hand = [];
    G.players["2"].hand = [];
    G.players["2"].playArea = ["later_solstice_card"];
    ...
    G.activeNationRulesets!["2"] = { ...G.activeNationRulesets!["2"], hookRules: [] };
  ...
  it("collapse during Solstice stops later Solstice effects and round advancement", () => {
    const G = createInitialState();
    G.unrestPile = [];
    G.players["1"].playArea = ["collapse_solstice_card"];
    G.players["1"].hand = [];
    G.players["2"].playArea = ["later_solstice_card"];
    G.players["2"].hand = [];
  ...
  it("lets a player choose Solstice order when one card can trigger Collapse", () => {
    const G = createInitialState();
    G.unrestPile = [];
    G.players["1"].playArea = ["collapse_solstice_card", "gain_before_collapse_solstice_card"];
    G.players["1"].hand = [];
    G.players["2"].hand = [];
    G.cardDb.collapse_solstice_card = {
      id: "collapse_solstice_card",
  ...
  it("pauses round handoff when a post-Solstice collapse override creates a pending choice", () => {
    ...
    expect(G.pendingChoice).toBeDefined();
    expect(G.pendingSolsticeRoundEnd).toEqual({ playerId: "2" });
    expect(G.players["1"].resources.knowledge).toBe(0);
    expect(G.round).toBe(2);
    expect(G.log.some((entry) => entry.message === "TurnPhase(turn_handoff): end_turn_complete")).toBe(false);
  ...
  it("does not complete a turn handoff after final scoring resolves at the Solstice boundary", () => {
    ...
    G.players["1"].discard = [];
    G.players["2"].discard = [];
    G.players["1"].resources.knowledge = 5;
    G.players["2"].resources.knowledge = 1;
    G.scoring = { reason: "main_deck_empty", triggeredBy: "1", phase: "final_round", finalRound: 1 };
  ...
  it("blocks normal actions while scoring finalization is waiting to resume", () => {
    const G = createInitialState();
    G.players["1"].hand = ["blocked_action"];
    G.players["1"].actionsRemaining = 1;
    G.players["1"].actionTokensAvailable = 1;
    ...
    expect(G.players["1"].hand).toEqual(["blocked_action"]);
    expect(G.players["1"].resources.knowledge).toBe(0);
  ...
  it.each([
    {
      ...
      setPending: (G: GameState) => { G.pendingPlayCardResolution = { playerId: "1", cardId: "blocked_action", freePlay: false }; }
    },
    {
      name: "played-card resolution",
      reason: "pending_played_card_resolution",
      setPending: (G: GameState) => { G.pendingPlayedCardResolution = { playerId: "1", cardId: "blocked_action", freePlay: false }; }
    },
    {
      name: "acquire-card resolution",
      reason: "pending_acquire_card_resolution",
      setPending: (G: GameState) => { G.pendingAcquireCardResolution = { playerId: "1", cardId: "market_action" }; }
    },
    {
      name: "nation-hook continuation",
      reason: "pending_nation_hook_continuation",
      setPending: (G: GameState) => { G.pendingNationHookContinuation = { playerId: "1", trigger: "after_play_card", nextIndex: 1, resolvedHookIndex: 0 }; }
    },
    {
      name: "Unrest-take continuation",
      reason: "pending_unrest_take_continuation",
      setPending: (G: GameState) => { G.pendingUnrestTakeContinuation = { playerId: "1", recipientPlayerIds: ["1"], countPerPlayer: 1, recipientIndex: 0, cardIndex: 0, taken: 0 }; }
    },
    {
      name: "Unrest-allocation resolution",
      reason: "pending_unrest_allocation_resolution",
      setPending: (G: GameState) => { G.pendingUnrestAllocationResolution = { playerId: "1", recipientPlayerIds: ["1"], availableUnrestCardIds: ["unrest_1"], nextIndex: 0 }; }
    },
    {
      name: "post-development resolution",
      reason: "pending_post_development_resolution",
      setPending: (G: GameState) => { G.pendingPostDevelopmentResolution = { playerId: "1", cardId: "development_1", resumeDrawCount: 1 }; }
    },
    {
      name: "reshuffle resolution",
      reason: "pending_reshuffle_resolution",
      setPending: (G: GameState) => { G.pendingReshuffleResolution = { playerId: "1", resumeDrawCount: 1 }; }
    },
    {
      name: "after-reshuffle effects",
      reason: "pending_after_reshuffle_effects",
      setPending: (G: GameState) => { G.pendingAfterReshuffleEffects = { playerId: "1", resumeDrawCount: 1, nextOverrideIndex: 0 }; }
    },
    {
      name: "reshuffle draw continuation",
      reason: "pending_reshuffle_draw",
      setPending: (G: GameState) => { G.pendingReshuffleDraw = { playerId: "1", resumeDrawCount: 1 }; }
    },
    {
      name: "Practice market-exile-before-cleanup continuation",
      reason: "pending_practice_market_exile_before_cleanup",
      setPending: (G: GameState) => { G.pendingPracticeMarketExileBeforeCleanup = { playerId: "1" }; }
    }
  ...
  it("keeps Collapse from an on-play effect instead of rolling the action back", () => {
    const G = createInitialState();
    G.unrestPile = [];
    G.players["1"].hand = ["collapse_action"];
    ...
    expect(G.players["1"].hand).not.toContain("collapse_action");
    expect(G.players["1"].playArea).toContain("collapse_action");
    expect(G.players["1"].discard).not.toContain("collapse_action");
    expect(G.players["1"].resources.knowledge).toBe(0);
  });
  ...
  it("allows a mandatory take-Unrest card to be played when the empty pile will Collapse", () => {
    const G = createInitialState();
    G.unrestPile = [];
    G.players["1"].hand = ["collapse_only_action"];
    addScoringUnrest(G, { "1": 1, "2": 2 });
  ...
  it("pauses Solstice for triggering-player allocation when multi-player Unrest runs short", () => {
    ...
    G.cardDb.short_unrest_solstice_card = {
      ...
      cost: 0,
      tags: [],
      effects: [{ trigger: "on_solstice", op: "take_unrest", targetPlayerIds: ["2", "1"], count: 1 } as any]
    };
    G.cardDb.later_solstice_card = {
      id: "later_solstice_card",
      displayName: "Later Solstice",
    ...
    resolveUnrestAllocationChoice({ G, ctx }, ["1"]);

    expect(G.pendingUnrestAllocationChoice).toBeUndefined();
    expect(G.players["1"].hand).toContain("only_unrest");
    expect(G.gameover?.reason).toBe("collapse:unrest_pile_empty");
    expect(G.players["2"].resources.knowledge).toBe(0);
  });
  ...
  it("spends an Exhaust token to resolve an exhaust ability from a card in play", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["exhaust_play_card"];
    G.players["1"].exhaustTokensAvailable = 1;
  ...
  it("returns an Exhaust token from a card in the play area", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["return_exhaust_card"];
    G.players["1"].exhaustTokensAvailable = 1;
  ...
  it("pauses to choose which Exhaust token to return before paying later costs, then resumes effects", () => {
    const G = createInitialState();
    G.players["1"].hand = ["return_exhaust_source"];
    G.players["1"].playArea = ["spent_a", "spent_b"];
    G.players["1"].actionTokensAvailable = 1;
    G.players["1"].actionsRemaining = 1;
    G.players["1"].exhaustTokensAvailable = 0;
    G.players["1"].resources.knowledge = 1;
    ...
    expect(G.players["1"].resources.knowledge).toBe(1);
    expect(G.players["1"].discard).toContain("return_exhaust_source");
  });
  ...
  it("does not play a Return Exhaust token action with no returnable token", () => {
    ...
    expect(G.players["1"].hand).toContain("return_exhaust_source");
    expect(G.players["1"].discard).not.toContain("return_exhaust_source");
    expect(G.players["1"].actionTokensAvailable).toBe(1);
  ...
  it("does not play a Market Exile action when all matching market cards have card-state tokens", () => {
    const G = createInitialState();
    G.players["1"].hand = ["market_exile_source"];
    G.players["1"].actionTokensAvailable = 1;
    G.players["1"].actionsRemaining = 1;
    ...
    expect(G.players["1"].hand).toEqual(["market_exile_source"]);
    expect(G.players["1"].discard).not.toContain("market_exile_source");
    expect(G.players["1"].actionsRemaining).toBe(1);
    expect(G.players["1"].actionTokensAvailable).toBe(1);
    expect(G.pendingExileChoice).toBeUndefined();
  ...
  it("does not play a play-area Exile action when all matching player cards have card-state tokens", () => {
    const G = createInitialState();
    G.players["1"].hand = ["player_exile_source"];
    ...
    expect(G.players["1"].hand).toEqual(["player_exile_source"]);
    expect(G.players["1"].discard).not.toContain("player_exile_source");
    expect(G.players["1"].actionsRemaining).toBe(1);
    expect(G.players["1"].actionTokensAvailable).toBe(1);
    expect(G.pendingExileChoice).toBeUndefined();
  ...
  it("auto-resolves an exhaust garrison ability with one host and one card in hand", () => {
    ...
    G.players["1"].playArea = ["test_region", "exhaust_garrison_card"];
    G.players["1"].hand = ["test_action_archive_survey"];
    G.players["1"].exhaustTokensAvailable = 1;
  ...
  it("auto-resolves an on-play Garrison Region using itself as the only host", () => {
    ...
    G.players["1"].hand = ["garrison_region", "test_action_archive_survey"];
    G.players["1"].playArea = [];

    playCard({ G, ctx }, "garrison_region");

    expect(G.pendingGarrisonChoice).toBeUndefined();
    expect(G.players["1"].playArea).toContain("garrison_region");
    expect(G.players["1"].hand).toEqual([]);
  ...
  it("does not exhaust a garrison ability when the explicit host is not a Region", () => {
    ...
    G.players["1"].playArea = ["test_region", "not_region", "exhaust_garrison_card"];
    G.players["1"].hand = ["test_action_archive_survey"];
    G.players["1"].exhaustTokensAvailable = 1;
  ...
  it("allows an exhaust discard-random ability with one card in hand", () => {
    ...
    G.players["1"].playArea = ["exhaust_discard_card"];
    G.players["1"].hand = ["test_action_archive_survey"];
    G.players["1"].exhaustTokensAvailable = 1;
  ...
  it("allows an exhaust remove-resource ability to resolve as much as possible with none available", () => {
    ...
    G.players["1"].playArea = ["exhaust_remove_card"];
    G.players["1"].resources.materials = 0;
    G.players["1"].exhaustTokensAvailable = 1;
  ...
  it("does not allow a reactive-only Exhaust ability to be used as an ordinary Exhaust", () => {
    ...
    G.players["1"].playArea = ["reactive_only_exhaust_card"];
    G.players["1"].resources.knowledge = 0;
    G.players["1"].exhaustTokensAvailable = 1;
  ...
  it("allows a reactive Exhaust ability to use selected payment", () => {
    ...
    G.players["1"].hand = ["resource_trigger_card"];
    G.players["1"].playArea = ["paid_reactive_exhaust_card"];
    G.players["1"].resources.knowledge = 0;
    G.players["1"].resources.influence = 0;
    G.players["1"].resources.goods = 1;
    G.players["1"].actionTokensAvailable = 1;
  ...
  it("keeps Collapse from an exhaust effect instead of rolling the exhaust back", () => {
    const G = createInitialState();
    G.unrestPile = [];
    G.players["1"].playArea = ["collapse_exhaust_card"];
    G.players["1"].exhaustTokensAvailable = 1;
    addScoringUnrest(G, { "1": 1, "2": 2 });
  ...
  it("cleanup clears only the current player's token markers across cleanup zones", () => {
    const G = createInitialState();
    G.players["1"].playArea = ["p0_play"];
    G.players["1"].powerArea = ["p0_power"];
    G.players["1"].stateArea = ["p0_state"];
    G.players["1"].nationDeck = ["p0_nation"];
    G.players["1"].developmentArea = ["p0_development"];
    G.players["1"].hand = [];
    G.players["2"].playArea = ["p1_play"];
    G.players["2"].hand = [];
    G.cardStates = {
  ...
  it("can resolve an exhaust ability from the Power card", () => {
    const G = createInitialState();
    G.players["1"].powerArea = ["exhaust_power_card"];
    G.players["1"].exhaustTokensAvailable = 1;
  ...
  it("does not use an Exhaust ability printed on a State card", () => {
    const G = createInitialState();
    G.players["1"].stateArea = ["exhaust_state_card"];
    G.players["1"].exhaustTokensAvailable = 1;
  ...
  it("Innovate discards hand, breaks through for an allowed suit, then enters cleanup", () => {
    ...
    expect(G.players["1"].discard).toEqual(["test_action_archive_survey", "test_action_scholars_circle"]);
  ...
  it("Innovate from market breaks through for the selected matching card", () => {
    ...
    expect(G.players["1"].hand).toContain("test_action_archive_survey");
    expect(G.players["1"].hand).not.toContain("test_action_foundry_shift");
    expect(G.market).toEqual(["test_action_foundry_shift"]);
  });
  ...
  it("does not open reactive Exhaust windows during Innovate Break-through gains or hooks", () => {
    ...
    G.players["1"].hand = ["test_action_archive_survey"];
    G.players["1"].playArea = ["innovate_reactive_exhaust"];
    G.players["1"].exhaustTokensAvailable = 1;
    G.cardDb.test_action_foundry_shift = { ...G.cardDb.test_action_foundry_shift, suit: "uncivilized" };
  ...
  it("Innovate pauses for an explicit Break Through choice when multiple market cards match", () => {
    ...
    expect(G.players["1"].discard).toEqual(["test_action_scholars_circle"]);
    expect(G.players["1"].hand).toEqual([]);
  ...
  it("Innovate can break through for Tributary", () => {
    ...
    expect(G.players["1"].discard).toEqual(["test_action_archive_survey"]);
    expect(G.players["1"].hand).toContain("visible_tributary");
  ...
  it("Revolt returns selected Unrest cards from hand to the Unrest pile and enters cleanup", () => {
    ...
    const G = createInitialState();
    G.market = [];
    G.players["1"].hand = ["test_unrest_1", "test_action_archive_survey"];
  ...
  it("Revolt leaves non-returned hand cards for the normal cleanup discard choice", () => {
    ...
    const G = createInitialState();
    G.market = [];
    G.players["1"].hand = ["test_unrest_1", "test_action_archive_survey", "test_action_scholars_circle"];
  ...
  it("does not open reactive Exhaust windows during Revolt passive hooks", () => {
    ...
    G.market = [];
    G.players["1"].hand = ["test_unrest_1", "test_action_archive_survey"];
    G.players["1"].playArea = ["revolt_reactive_exhaust"];
    G.players["1"].exhaustTokensAvailable = 1;
  ...
  it("honors selected Progress substitution when acquiring from the market", () => {
    ...
    expect(G.players["1"].resources.knowledge).toBe(0);
    expect(G.players["1"].resources.goods).toBe(1);
    expect(G.players["1"].hand).toContain("progress_paid_market_card");
    expect(G.market).toEqual([]);
  });
  ...
  it("does not acquire from the market when selected payment overpays the cost", () => {
    ...
    expect(G.players["1"].resources.knowledge).toBe(1);
    expect(G.players["1"].resources.goods).toBe(1);
    expect(G.players["1"].hand).not.toContain("progress_paid_market_card");
  ...
  it("requires direct Progress and Goods for structured market acquire costs", () => {
    ...
    expect(G.players["1"].hand).not.toContain("structured_cost_card");
    ...
    expect(G.players["1"].resources.knowledge).toBe(0);
    expect(G.players["1"].resources.goods).toBe(0);
    expect(G.players["1"].hand).toContain("structured_cost_card");
    expect(G.market).toEqual([]);
  });
  ...
  it("failed resumed tucked-Unrest hooks during card-effect Market acquisition restore the card play state", () => {
    ...
    G.activeNationRulesets!["1"] = {
      ...G.activeNationRulesets!["1"],
      hookRules: [
        ...G.activeNationRulesets!["1"].hookRules,
        {
        trigger: "after_gain_unrest",
    ...
    expect(G.players["1"].hand).not.toContain("test_action_foundry_shift");
    expect(G.players["1"].hand).not.toContain("test_unrest_1");
    expect(G.players["1"].hand).not.toContain("test_unrest_2");
  ...
  it("pauses later nation hooks when an earlier hook creates a discard choice", () => {
    ...
    G.marketRefillPool = [];
    G.marketDecks = undefined;
    G.players["1"].hand = ["discard_hook_card_a", "discard_hook_card_b"];
    G.players["1"].resources.materials = 1;
    G.players["1"].resources.knowledge = 0;
    ...
    expect(G.pendingDiscardChoice).toBeUndefined();
    expect(G.players["1"].discard).toContain("discard_hook_card_a");
    expect(G.players["1"].resources.knowledge).toBe(1);
    expect(G.log.some((entry) => entry.message === "Nation hook after_acquire #0 resolved.")).toBe(true);
    expect(G.log.some((entry) => entry.message === "Nation hook after_acquire #1 resolved.")).toBe(true);
  });
  ...
  it("resolves the selected pending Find choice", () => {
    ...
    expect(G.players["1"].hand).toEqual([]);
    expect(G.players["1"].discard).toEqual(["test_action_archive_survey", "test_action_foundry_shift"]);
  ...
  it("routes pending Find choices with a History destination through no-History discard replacement", () => {
    ...
    expect(G.players["1"].hand).toEqual([]);
    expect(G.players["1"].history).toEqual([]);
    expect(G.players["1"].discard).toEqual(["found_card"]);
  ...
  it("resolves pending Find choices selected from a nation History replacement zone", () => {
    const G = createInitialState();
    G.players["1"].sideAreas = { sunken: ["found_card"] };
    G.cardDb.found_card = {
      id: "found_card",
      displayName: "Found Card",
      type: "action",
  ...
  it("plays Find text that sources History from a nation History replacement zone", () => {
    ...
    expect(G.players["1"].history).toEqual([]);
    expect(G.players["1"].sideAreas?.sunken).toEqual([]);
    expect(G.players["1"].discard).toEqual(["found_card", finder]);
    expect(G.players["1"].actionsRemaining).toBe(0);
    expect(G.players["1"].actionTokensAvailable).toBe(0);
  });
  ...
  it("resumes remaining effects after resolving a pending Find choice", () => {
    ...
    expect(G.pendingFindChoice).toBeUndefined();
    expect(G.players["1"].discard).toContain("test_action_foundry_shift");
    expect(G.players["1"].resources.materials).toBe(2);
  });
  ...
  it("matches source-suited reactive Exhausts against a garrisoned card that released resources", () => {
    ...
    expect(G.players["1"].discard).toContain(childCardId);
    expect(G.players["1"].resources.knowledge).toBe(1);
    expect(G.pendingReactiveExhaustChoice).toMatchObject({
      playerId: "1",
      cardIds: [exhaustCardId],
    ...
    expect(G.players["1"].resources.influence).toBe(1);
    expect(G.players["1"].resources.materials).toBe(1);
    expect(G.players["1"].discard).toContain(finderCardId);
  });
  ...
  it("resolves the selected pending Return Unrest choice", () => {
    ...
    expect(G.pendingReturnUnrestChoice).toBeUndefined();
    expect(G.players["1"].hand).toEqual(["hand_unrest"]);
    expect(G.players["1"].discard).toEqual([]);
  ...
  it("resolves the selected pending Place-on-deck choice", () => {
    ...
    expect(G.pendingPlaceOnDeckChoice).toBeUndefined();
    expect(G.players["1"].hand).toEqual(["first_card"]);
    expect(G.players["1"].deck).toEqual(["second_card", "existing_top"]);
    expect(G.players["1"].resources.materials).toBe(1);
  ...
  it("resolves the selected pending Give-card choice", () => {
    ...
    resolveGiveCardChoice({ G, ctx }, "second_card", "2");

    expect(G.pendingGiveCardChoice).toBeUndefined();
    expect(G.players["1"].hand).toEqual(["first_card"]);
    expect(G.players["2"].hand).toEqual(["opponent_card", "second_card"]);
    expect(G.players["1"].resources.knowledge).toBe(1);
    expect(G.log.at(-1)?.message).toBe("GiveCardChoiceResolved(giver/second_card->2)");
  });
  ...
  it("resolves the selected pending Exile acquisition choice", () => {
    ...
    G.players["1"].playArea = ["reactive_exile_choice_unrest_exhaust"];
    G.players["1"].resources.influence = 0;
    G.players["1"].resources.knowledge = 0;
  ...
  it("failed resumed after-gain-Unrest hooks during short-pile allocation restore the allocation choice", () => {
    ...
    resolveUnrestAllocationChoice({ G, ctx }, ["1", "2"]);
  ...
  it("pauses a selected Market gain for resource reactive Exhausts before resume effects", () => {
    ...
    G.players["1"].playArea = ["market_choice_reactive_exhaust"];
    G.players["1"].exhaustTokensAvailable = 1;
    G.players["1"].resources.knowledge = 0;
    G.players["1"].resources.materials = 0;
    G.players["1"].resources.influence = 0;
    ...
    expect(G.players["1"].discard).toContain("market_civilized");
    expect(G.players["1"].resources.knowledge).toBe(1);
    expect(G.players["1"].resources.influence).toBe(0);
    expect(G.pendingReactiveExhaustChoice).toMatchObject({
      playerId: "1",
  ...
  it("resolves the selected pending market Exile choice", () => {
    ...
    expect(G.players["1"].exile).toContain("market_civilized");
    expect(G.unrestPile).toContain("test_unrest_1");
    expect(G.marketUnrest.market_civilized).toBeUndefined();
    expect(G.marketUnrest.market_refill).toEqual(["test_unrest_2"]);
  ...
  it("resolves the selected pending History Exile choice", () => {
    ...
    expect(G.pendingExileChoice).toBeUndefined();
    expect(G.players["1"].history).toEqual([]);
    expect(G.players["1"].exile).toEqual(["history_card"]);
  ...
  it("resolves a pending History Exile choice from a nation History replacement zone", () => {
    const G = createInitialState();
    G.players["1"].history = [];
    G.players["1"].sideAreas = { sunken: ["history_card"] };
    G.activeNationRulesets!["1"] = {
      ...G.activeNationRulesets!["1"],
      zoneOverrides: [{ op: "replace_history_with_zone", zoneId: "sunken", displayName: "Sunken", cardsScore: true } as any]
    };
  ...
  it("resolves the selected pending discard Exile choice", () => {
    ...
    expect(G.pendingExileChoice).toBeUndefined();
    expect(G.players["1"].discard).toEqual([]);
    expect(G.players["1"].exile).toEqual(["discard_card"]);
  ...
  it("stops pending Exile acquisition cleanly when required Unrest causes Collapse", () => {
    ...
    expect(G.pendingAcquireChoice).toBeUndefined();
    expect(G.players["1"].exile).toEqual(["exiled_civilized"]);
    expect(G.players["1"].hand).not.toContain("exiled_civilized");
    expect(G.players["1"].resources.knowledge).toBe(0);
  ...
  it("continues multi-card acquire choices so newly refilled matching cards can be acquired", () => {
    ...
    runEffects({ G, playerId: "1", selfCardId: "multi_acquirer" }, [{
      trigger: "on_play",
      op: "acquire_card",
      source: "market",
      suit: "civilized",
    ...
    expect(G.players["1"].hand).toContain("market_civilized_b");
    ...
    expect(G.pendingAcquireChoice).toBeUndefined();
    expect(G.players["1"].hand).toEqual(expect.arrayContaining(["market_civilized_b", "market_civilized_refill"]));
    expect(G.players["1"].hand).not.toContain("market_civilized_a");
  ...
  it("resolves the selected pending public setup Exile Break Through choice", () => {
    ...
    expect(G.pendingBreakThroughChoice).toBeUndefined();
    expect(G.globalSpecialZones.exile.cardIds).toEqual([]);
    expect(G.players["1"].hand).toContain("setup_exiled_civilized");
    expect(G.players["1"].discard).not.toContain("test_unrest_1");
    expect(G.unrestPile).toEqual(["test_unrest_1"]);
  ...
  it("resolves the selected pending Market Break Through choice", () => {
    ...
    expect(G.players["1"].hand).toContain("market_uncivilized_b");
    expect(G.players["1"].hand).not.toContain("test_unrest_1");
    expect(G.players["1"].resources.knowledge).toBe(1);
    expect(G.unrestPile).toContain("test_unrest_1");
    expect(G.marketUnrest.market_refill).toEqual(["test_unrest_2"]);
  ...
  it("continues multi-card market Break Through choices after refilling matching cards", () => {
    ...
    runEffects({ G, playerId: "1", selfCardId: "multi_breaker" }, [{
      trigger: "on_play",
      op: "break_through",
      source: "market",
      suit: "uncivilized",
    ...
    expect(G.pendingBreakThroughChoice).toBeUndefined();
    expect(G.players["1"].hand).toEqual(expect.arrayContaining(["market_uncivilized_b", "market_uncivilized_refill"]));
    expect(G.players["1"].hand).not.toContain("market_uncivilized_a");
  ...
  it("pauses counted pending Region choices for reactive Exhaust windows before the next Region selection", () => {
    ...
    G.players["1"].playArea = [firstRegion, secondRegion, exhaustCardId];
    G.players["1"].exhaustTokensAvailable = 1;
    ...
    expect(G.players["1"].hand).toContain(firstRegion);
    expect(G.players["1"].resources.materials).toBe(1);
    expect(G.pendingRegionChoice).toBeUndefined();
    expect(G.pendingReactiveExhaustChoice).toMatchObject({
      playerId: "1",
    ...
    expect(G.pendingRegionChoice).toBeUndefined();
    expect(G.players["1"].hand).toEqual(expect.arrayContaining([firstRegion, secondRegion]));
    expect(G.players["1"].resources.influence).toBe(1);
