...
import { acquireFromExile, availableExileCards, canAcquireExileCard, exileMarketCard, exilePlayerCard, marketCardHasTokens, playerCardOrGarrisonHasTokens, playerExileSourceCards } from "../game/exile";
import { acquireMarketCard, gainMarketCard, takeMarketCard } from "../game/marketAcquire";
import { drawFameCardResult, isFameCard, peekFameCards, returnFameCard, takeFameCardResult, zoneCardsForReturnFame } from "../game/fame";
...
import { returnableExhaustTokenCardIds, returnExhaustToken } from "../game/exhaustTokens";
...
interface Ctx {
  ...
  allowPendingScoringFinalizationEffects?: boolean;
}
...
function playerResourceGainsSince(G: GameState, playerId: string, before: Record<ResourceName, number>): Partial<Record<ResourceName, number>> {
  const gainedResources: Partial<Record<ResourceName, number>> = {};
  for (const resource of RESOURCE_NAMES) {
    const delta = resourceAmount(G.players[playerId]?.resources, resource) - before[resource];
...
function resolveOrQueueDeferredDevelopment(ctx: Ctx, effect: Extract<Effect, { op: "develop" }> | undefined): void {
  ...
  if (pending) {
    pending.resumeEffects = [effect, ...(pending.resumeEffects ?? [])];
    return;
  }
  createCardDrivenDevelopmentChoice(ctx.G, ctx.playerId, effect.sourceCardId ?? ctx.selfCardId, { free: effect.free });
}
...
type ResumablePendingChoice =
  ...
  | NonNullable<GameState["pendingBreakThroughEffectResolution"]>;
...
function pendingEffectInterruption(G: GameState): ResumablePendingChoice | undefined {
  return G.pendingChoice
    ...
    ?? (G.pendingRegionChoiceContinuation?.resolving ? undefined : G.pendingRegionChoiceContinuation)
    ...
    ?? (G.pendingBreakThroughEffectResolution?.resolving ? undefined : G.pendingBreakThroughEffectResolution);
}

function pendingNonResumableEffectBoundary(ctx: Ctx): boolean {
  return Boolean(ctx.G.pendingScoringFinalization && !ctx.allowPendingScoringFinalizationEffects);
}

function runEffectNationHooks(ctx: Ctx, trigger: NationHookTrigger, cardIds: string[]): boolean {
  for (const cardId of cardIds) {
    const resolved = runNationHooksWithEffectRunner({
      G: ctx.G,
      playerId: ctx.playerId,
      trigger,
      payload: { cardId },
      runHookEffects: (_ruleset, _hookIndex, effects) => runEffects(ctx, effects as any)
    });
    if (!resolved) return false;
    if (ctx.G.gameover || pendingNonResumableEffectBoundary(ctx) || pendingEffectInterruption(ctx.G)) return true;
  }
  return true;
}

function runBreakThroughNationHooks(ctx: Ctx, cardIds: string[], resumeEffects?: Effect[]): boolean {
  for (let index = 0; index < cardIds.length; index += 1) {
    ...
    if (ctx.G.gameover || pendingEffectInterruption(ctx.G)) {
      ctx.G.pendingBreakThroughEffectResolution = {
        playerId: ctx.playerId,
        sourceCardId: ctx.selfCardId,
        gainedCardIds: cardIds,
        afterBreakThroughCardReactiveChecked: true,
        nextAfterBreakThroughReactiveCardIndex: cardIds.length,
        nextAfterBreakThroughHookCardIndex: index + 1,
        ...(resumeEffects?.length ? { resumeEffects } : {})
      };
      return true;
    }
  }
...
type ReactiveExhaustEvent =
  | { trigger: "after_gain_resource"; resource?: ResourceName; sourceCardId?: string; sourceWasInPlay?: boolean }
  | { trigger: "after_take_unrest"; targetPlayerId: string }
  | { trigger: "after_acquire_card"; targetPlayerId: string }
  | { trigger: "after_play_card"; targetPlayerId: string }
  | { trigger: "after_break_through_card"; targetPlayerId: string };

function conditionMatchesReactiveExhaust(G: GameState, condition: ReactiveExhaustCondition | undefined, event: ReactiveExhaustEvent, ownerPlayerId: string): boolean {
  ...
  if (condition.trigger === "after_gain_resource" && event.trigger === "after_gain_resource") {
    if (condition.resource && canonicalResourceName(condition.resource) !== (event.resource ? canonicalResourceName(event.resource) : undefined)) return false;
    if (!condition.sourceSuit) return true;
    if (!event.sourceCardId || !event.sourceWasInPlay) return false;
    return cardHasSuitIconForPlayer(G, ownerPlayerId, G.cardDb[event.sourceCardId], condition.sourceSuit);
  }
  ...
  if (condition.trigger === "after_play_card" && event.trigger === "after_play_card") {
    const target = condition.target ?? "any";
    if (target === "self") return event.targetPlayerId === ownerPlayerId;
    if (target === "opponent") return event.targetPlayerId !== ownerPlayerId;
    return true;
  }
  if (condition.trigger === "after_break_through_card" && event.trigger === "after_break_through_card") {
    const target = condition.target ?? "any";
    if (target === "self") return event.targetPlayerId === ownerPlayerId;
    if (target === "opponent") return event.targetPlayerId !== ownerPlayerId;
    return true;
      ...
      !isCardExhausted(ctx.G, cardId)
        && (ctx.G.cardDb[cardId]?.effects ?? []).some((effect) =>
          effect.trigger === "on_exhaust"
            && conditionMatchesReactiveExhaust(ctx.G, reactiveCondition(effect), event, playerId)
...
export function createReactiveExhaustChoice(ctx: Ctx, event: ReactiveExhaustEvent): void {
  ...
  const canonicalEventResource = event.trigger === "after_gain_resource" && event.resource
    ? canonicalResourceName(event.resource)
    : undefined;
  ...
  ctx.G.pendingReactiveExhaustChoice = {
    ...
    ...(event.trigger === "after_gain_resource" ? { resource: canonicalEventResource } : {}),
    ...(event.trigger === "after_gain_resource" ? { eventSourceCardId: event.sourceCardId, eventSourceWasInPlay: event.sourceWasInPlay } : {}),
    ...("targetPlayerId" in event ? { targetPlayerId: event.targetPlayerId } : {})
  };
  const detail = event.trigger === "after_gain_resource" ? canonicalEventResource ?? "any" : event.targetPlayerId;
...
function createReactiveExhaustChoicesForResourceGainSources(ctx: Ctx, sources: ResourceGainSource[], fallbackGains: Partial<Record<ResourceName, number>> = {}): void {
  if (sources.length === 0) {
...
function runCommerceEffects(ctx: Ctx, tradeRouteCardId: string, ownerPlayerId = ctx.playerId): boolean {
  const effects = commerceEffects(ctx.G, tradeRouteCardId);
  return runEffects({ ...ctx, playerId: ownerPlayerId, selfCardId: tradeRouteCardId }, effects);
}

function commerceEffects(G: GameState, tradeRouteCardId: string): Extract<Effect, { op: "commerce" }>[] {
  return (G.cardDb[tradeRouteCardId]?.effects ?? []).filter((candidate): candidate is Extract<Effect, { op: "commerce" }> => {
...
function moveFoundCard(
  ...
  G.players[playerId][destination].push(...movedCardIds);
  return destination;
}

function cardMatchesFindCriteria(G: GameState, playerId: string, cardId: string, effect: Extract<Effect, { op: "find_card" }>): boolean {
  const card = G.cardDb[cardId];
...
function cardMatchesAcquireCriteria(G: GameState, playerId: string, cardId: string, effect: Extract<Effect, { op: "acquire_card" }>): boolean {
  const card = G.cardDb[cardId];
  if (!card) return false;
...
function cardMatchesExileCriteria(G: GameState, playerId: string, cardId: string, effect: Extract<Effect, { op: "exile_card" }>, source: Extract<Effect, { op: "exile_card" }>["source"]): boolean {
  const card = G.cardDb[cardId];
  if (!card) return false;
...
function runAcquireReactiveWindows(
  ctx: Ctx,
  cardId: string,
  takenUnrestPlayerIds: string[],
  collectedResources: Partial<Record<ResourceName, number>>,
  collectedResourceSources: ResourceGainSource[],
  resumeEffects: Effect[] | undefined
): boolean {
  for (let index = 0; index < takenUnrestPlayerIds.length; index += 1) {
    ...
    if (pendingEffectInterruption(ctx.G)) {
      ctx.G.pendingAcquireEffectResolution = {
        playerId: ctx.playerId,
        cardId,
        sourceCardId: ctx.selfCardId,
  ...
  createReactiveExhaustChoice(ctx, { trigger: "after_acquire_card", targetPlayerId: ctx.playerId });
  if (pendingEffectInterruption(ctx.G)) {
    appendResumeEffectsToPending(ctx.G, resumeEffects);
    return true;
  }
  return false;
}

function cardMatchesMarketMoveCriteria(G: GameState, playerId: string, cardId: string, effect: Extract<Effect, { op: "gain_card" | "take_card" }>): boolean {
  const card = G.cardDb[cardId];
...
function matchingMarketMoveCards(G: GameState, playerId: string, effect: Extract<Effect, { op: "gain_card" | "take_card" }>): string[] {
  return G.market.filter((marketCardId) => cardMatchesMarketMoveCriteria(G, playerId, marketCardId, effect));
}
...
function runMarketMoveReactiveWindows(
  ctx: Ctx,
  takenUnrestPlayerIds: string[],
  collectedResources: Partial<Record<ResourceName, number>>,
  collectedResourceSources: ResourceGainSource[],
  ...
  createReactiveExhaustChoicesForResourceGainSources(ctx, collectedResourceSources, collectedResources);
  if (pendingEffectInterruption(ctx.G)) {
    ctx.G.pendingMarketMoveEffectResolution = {
      playerId: ctx.playerId,
      sourceCardId: ctx.selfCardId,
      takenUnrestPlayerIds,
      collectedResources: {},
...
function cardMatchesBreakThroughCriteria(G: GameState, playerId: string, cardId: string, effect: Extract<Effect, { op: "break_through" }>): boolean {
  if (effect.cardId && cardId !== effect.cardId) return false;
  return cardHasSuitIconForPlayer(G, playerId, G.cardDb[cardId], effect.suit);
}
...
function matchingMarketBreakThroughCards(G: GameState, playerId: string, effect: Extract<Effect, { op: "break_through" }>): string[] {
  return G.market.filter((marketCardId) => cardMatchesBreakThroughCriteria(G, playerId, marketCardId, effect));
}
...
function shuffleFindDeckIfNeeded(G: GameState, playerId: string, zone: "deck" | "nationDeck", randomNumber?: () => number): void {
  ...
  const searchableCards = player.nationDeck.filter((cardId) => !isEffectiveAccessionCard(G, playerId, player, cardId));
...
function searchableFindCards(G: GameState, playerId: string, player: PlayerState, zone: string): string[] {
  ...
  return player.nationDeck.filter((cardId) => !isEffectiveAccessionCard(G, playerId, player, cardId));
}

function lookableCards(G: GameState, playerId: string, player: PlayerState, source: LookSourceZone): string[] {
  if (source === "deck") return [...player.deck];
  if (source === "nationDeck") return lookableNationDeckCards(G, player, playerId);
...
function removeFromDrawSource(G: GameState, playerId: string, source: Exclude<DrawSourceZone, "deck" | "fameDeck">, cardId: string): boolean {
  const remove = (cards: string[] | undefined): boolean => {
    const index = cards?.indexOf(cardId) ?? -1;
    if (!cards || index < 0) return false;
    cards.splice(index, 1);
    return true;
  };
  if (source === "exile") {
    return remove(G.players[playerId].exile) || remove(G.globalSpecialZones?.exile?.cardIds);
  }
  return remove(G.players[playerId][source]);
}
  ...
  if (effect.cardId) {
    ...
    for (const zone of zones) {
      for (const resolvedZone of resolvedFindZones(ctx.G, ctx.playerId, zone)) {
        ...
        const destination = moveFoundCard(ctx.G, ctx.playerId, resolvedZone, effect.destination, effect.cardId, collectedResources, collectedResourceSources);
...
function drawFameCards(ctx: Ctx, count: number): void {
  ...
  for (let i = 0; i < count; i += 1) {
    const resourcesBefore = playerResourceSnapshot(ctx.G, ctx.playerId);
    const result = drawFameCardResult(ctx.G, ctx.playerId, { deferDevelopmentChoice: true });
    const cardId = result.cardId;
    if (!cardId) break;
    drawn.push(cardId);
    const gainedResources = playerResourceGainsSince(ctx.G, ctx.playerId, resourcesBefore);
    if (!ctx.G.gameover) createReactiveExhaustChoicesForResourceGains(ctx, gainedResources);
    if (!ctx.G.gameover) resolveOrQueueDeferredDevelopment(ctx, result.deferredDevelopmentEffect);
...
function canCreateCostBoundary(ctx: Ctx, effect: Effect): boolean {
...
function firstChoiceBoundary(ctx: Ctx, effects: Effect[]): number {
  return effects.findIndex((effect) => canCreateCostBoundary(ctx, effect));
}

function costPaymentPrefix(ctx: Ctx, effects: Effect[]): Effect[] {
  const boundary = firstChoiceBoundary(ctx, effects);
...
function canResolveReturnFameEffect(ctx: Ctx, effect: Extract<Effect, { op: "return_fame" }>): boolean {
  const sourceZones: ReturnFameSourceZone[] = effect.sourceZones?.length ? effect.sourceZones : ["hand"];
  return sourceZones.some((zone) => zoneCardsForReturnFame(ctx.G, ctx.playerId, zone).some((cardId) =>
    isFameCard(ctx.G, cardId) && (!effect.cardId || cardId === effect.cardId)
  ));
}
...
function canResolveReturnExhaustTokenEffect(ctx: Ctx, effect: Extract<Effect, { op: "return_exhaust_token" }>): boolean {
  const cardIds = returnableExhaustTokenCardIds(ctx.G, ctx.playerId);
  if (
    effect.trigger === "on_exhaust"
    && ctx.selfCardId
    && ctx.G.players[ctx.playerId].playArea.includes(ctx.selfCardId)
    && (!effect.cardId || effect.cardId === ctx.selfCardId)
  ) {
    return true;
  }
...
function canResolveExileEffect(ctx: Ctx, effect: Extract<Effect, { op: "exile_card" }>): boolean {
  return isPlayerExileSource(effect.source)
    ? playerExileSourceCards(ctx.G, ctx.playerId, effect.source).some((cardId) => cardMatchesExileCriteria(ctx.G, ctx.playerId, cardId, effect, effect.source))
    : ctx.G.market.some((cardId) => cardMatchesExileCriteria(ctx.G, ctx.playerId, cardId, effect, "market"));
}

function canResolveAcquireEffect(ctx: Ctx, effect: Extract<Effect, { op: "acquire_card" }>): boolean {
  return effect.source === "exile"
    ? availableExileCards(ctx.G, ctx.playerId).some((cardId) => canAcquireExileCard(ctx.G, cardId) && cardMatchesAcquireCriteria(ctx.G, ctx.playerId, cardId, effect))
    : ctx.G.market.some((cardId) => cardMatchesAcquireCriteria(ctx.G, ctx.playerId, cardId, effect));
}
...
function canResolveFindEffect(ctx: Ctx, effect: Extract<Effect, { op: "find_card" }>): boolean {
  ...
  return zones.some((zone) => resolvedFindZones(ctx.G, ctx.playerId, zone).some((resolvedZone) =>
    searchableFindCards(ctx.G, ctx.playerId, p, resolvedZone).some((cardId) => cardMatchesFindCriteria(ctx.G, ctx.playerId, cardId, effect))
  ));
}
...
function canResolveExactDeckBreakThroughEffect(ctx: Ctx, suit: Suit, cardId: string): boolean {
  if (!cardHasSuitIconForPlayer(ctx.G, ctx.playerId, ctx.G.cardDb[cardId], suit)) return false;
  if (suit === "tributary" && visibleTributaryBreakThroughCards(ctx.G, ctx.playerId).includes(cardId)) return true;
  const sourceDeck = suit === "tributary" ? undefined : deckForSuit(suit);
  if (sourceDeck && marketDeckDrawableTopCard(ctx.G, sourceDeck) === cardId) return true;
  return (ctx.G.marketDecks?.mainDeck ?? []).includes(cardId);
}

function canResolveDeckBreakThroughEffect(ctx: Ctx, suit: Suit, cardId?: string): boolean {
  if (cardId) return canResolveExactDeckBreakThroughEffect(ctx, suit, cardId);
  if (suit === "tributary" && visibleTributaryBreakThroughCards(ctx.G, ctx.playerId).length > 0) return true;
  const sourceDeck = suit === "tributary" ? undefined : deckForSuit(suit);
  if (sourceDeck && marketDeckHasDrawableCard(ctx.G, sourceDeck)) return true;
  const mainDeck = ctx.G.marketDecks?.mainDeck ?? [];
  if (mainDeck.some((cardId) => cardHasSuitIconForPlayer(ctx.G, ctx.playerId, ctx.G.cardDb[cardId], suit))) return true;
...
function canResolveEffectText(ctx: Ctx, effect: Effect): boolean {
  ...
  switch (effect.op) {
    ...
    case "develop":
      return effect.free
        ? canUseDevelopmentArea(ctx.G, ctx.playerId) && ctx.G.players[ctx.playerId].developmentArea.length > 0
        : canPayAnyDevelopmentCard(ctx.G, ctx.playerId);
    case "exile_card":
    ...
    case "gain_card":
    case "take_card":
      return matchingMarketMoveCards(ctx.G, ctx.playerId, effect).length > 0;
    case "break_through":
      return effect.source === "market"
        ? matchingMarketBreakThroughCards(ctx.G, ctx.playerId, effect).length > 0
        : effect.source === "exile"
          ? availableExileCards(ctx.G, ctx.playerId).some((cardId) => cardMatchesBreakThroughCriteria(ctx.G, ctx.playerId, cardId, effect))
          : canResolveDeckBreakThroughEffect(ctx, effect.suit, effect.cardId);
    case "find_card":
...
export function runEffects(ctx: Ctx, effects: Effect[]): boolean {
  ...
  const prefix = costPaymentPrefix(ctx, effects);
  ...
  if (costEntries.length === 1) {
    ...
    if (!payResourceCost(ctx.G, ctx.playerId, resource, amount, ctx.randomNumber)) return false;
  } else if (costEntries.length > 1) {
    if (!payResourceCosts(ctx.G, ctx.playerId, cost, undefined, ctx.randomNumber)) return false;
  }
  if (ctx.G.gameover || pendingNonResumableEffectBoundary(ctx) || pendingEffectInterruption(ctx.G)) return true;
  ...
  for (let index = 0; index < payableEffects.length; index += 1) {
    ...
    if (ctx.G.gameover || pendingNonResumableEffectBoundary(ctx)) return true;
    const pending = pendingEffectInterruption(ctx.G);
    if (pending) {
...
function runEffect(ctx: Ctx, effect: Effect): boolean {
  ...
  switch (effect.op) {
    ...
    case "return_unrest": {
      ...
      if (cardIds.length === 1) {
        if (!returnUnrestCard(ctx.G, ctx.playerId, cardIds[0], sourceZones)) {
          ctx.G.log.push({ round: ctx.G.round, playerId: ctx.playerId, message: `ReturnUnrestFailed(${cardIds[0]})` });
          return false;
        }
        break;
      }
    ...
    case "return_fame": {
      const sourceZones: ReturnFameSourceZone[] = effect.sourceZones?.length ? effect.sourceZones : ["hand"];
      if (effect.cardId) {
        if (!returnFameCard(ctx.G, ctx.playerId, effect.cardId, sourceZones)) {
          ctx.G.log.push({ round: ctx.G.round, playerId: ctx.playerId, message: `ReturnFameFailed(${effect.cardId})` });
          return false;
        }
        break;
      }
      const cardIds = sourceZones
        .flatMap((zone) => zoneCardsForReturnFame(ctx.G, ctx.playerId, zone))
        .filter((cardId, index, all) => isFameCard(ctx.G, cardId) && all.indexOf(cardId) === index);
      if (cardIds.length === 0) {
        ctx.G.log.push({ round: ctx.G.round, playerId: ctx.playerId, message: "ReturnFameSkipped(no_eligible_fame)" });
        break;
      }
      if (cardIds.length === 1) {
        if (!returnFameCard(ctx.G, ctx.playerId, cardIds[0], sourceZones)) {
          ctx.G.log.push({ round: ctx.G.round, playerId: ctx.playerId, message: `ReturnFameFailed(${cardIds[0]})` });
          return false;
        }
        break;
      }
      ctx.G.pendingReturnFameChoice = { playerId: ctx.playerId, sourceCardId: ctx.selfCardId, cardIds, sourceZones };
      ctx.G.log.push({ round: ctx.G.round, playerId: ctx.playerId, message: `ReturnFameChoicePending(${ctx.selfCardId ?? "unknown"}/options=${cardIds.length})` });
      break;
    }
    case "place_card_on_deck": {
      ...
      if (cardIds.length === 1) {
        if (!placeCardOnDeck(ctx.G, ctx.playerId, cardIds[0], sourceZone)) {
          ctx.G.log.push({ round: ctx.G.round, playerId: ctx.playerId, message: `PlaceOnDeckFailed(${cardIds[0]}/${sourceZone})` });
          return false;
        }
        break;
      }
    ...
    case "return_exhaust_token": {
      const cardIds = effect.cardId
        ? returnableExhaustTokenCardIds(ctx.G, ctx.playerId).filter((cardId) => cardId === effect.cardId)
        : returnableExhaustTokenCardIds(ctx.G, ctx.playerId);
      if (cardIds.length === 0) {
        if (effect.cardId) {
          ctx.G.log.push({ round: ctx.G.round, playerId: ctx.playerId, message: `ReturnExhaustTokenFailed(${effect.cardId})` });
          return false;
        }
        ctx.G.log.push({ round: ctx.G.round, playerId: ctx.playerId, message: "ReturnExhaustTokenSkipped(no_eligible_card)" });
        break;
      }
      if (!effect.cardId && cardIds.length > 1) {
        ctx.G.pendingReturnExhaustTokenChoice = { playerId: ctx.playerId, sourceCardId: ctx.selfCardId, cardIds };
        ctx.G.log.push({ round: ctx.G.round, playerId: ctx.playerId, message: `ReturnExhaustTokenChoicePending(${ctx.selfCardId ?? "unknown"}/options=${cardIds.length})` });
        break;
      }
    ...
    case "give_card": {
      ...
      if (cardIds.length === 1 && recipientPlayerIds.length === 1) {
        if (!giveCardToPlayer(ctx.G, ctx.playerId, cardIds[0], recipientPlayerIds[0])) {
          ctx.G.log.push({ round: ctx.G.round, playerId: ctx.playerId, message: `GiveCardFailed(${cardIds[0]}->${recipientPlayerIds[0]})` });
          return false;
        }
        break;
      }
    ...
    case "swap_card": {
      ...
      if (choices.length === 1) {
        ...
        if (!swapCardWithMarket(ctx.G, { playerId: ctx.playerId, sourceZone, cardId: choice.cardId, marketCardId: choice.marketCardId })) {
          ctx.G.log.push({ round: ctx.G.round, playerId: ctx.playerId, message: `SwapFailed(${choice.cardId}<->${choice.marketCardId}/source=${sourceZone})` });
          return false;
        }
        break;
      }
    ...
    case "gain_fame": {
      ...
      for (let i = 0; i < effect.count; i += 1) {
        const resourcesBefore = playerResourceSnapshot(ctx.G, ctx.playerId);
        const result = takeFameCardResult(ctx.G, ctx.playerId, { deferDevelopmentChoice: true });
        const cardId = result.cardId;
        if (!cardId) break;
        gained.push(cardId);
        const gainedResources = playerResourceGainsSince(ctx.G, ctx.playerId, resourcesBefore);
    ...
    case "treat_suit_as": {
      if (effect.to.length === 0) return false;
      ctx.G.treatedSuitIconsThisTurn ??= {};
      ctx.G.treatedSuitIconsThisTurn[ctx.playerId] ??= [];
      ctx.G.treatedSuitIconsThisTurn[ctx.playerId].push({ from: effect.from, to: [...effect.to] });
      ctx.G.log.push({ round: ctx.G.round, playerId: ctx.playerId, message: `TreatSuitAs(${effect.from}->${effect.to.join("|")})` });
      break;
    }
    case "profit": {
      ...
      const collectedResources: Partial<Record<ResourceName, number>> = {};
      const collectedResourceSources: ResourceGainSource[] = [];
      recordResourceGains(collectedResources, collectedResourceSources, ctx.selfCardId, true, collectCardResourcesToPlayer(ctx.G, ctx.playerId, ctx.selfCardId));
      const garrisonedCardIds = detachGarrisonedCards(ctx.G, ctx.selfCardId);
      garrisonedCardIds.forEach((cardId) => recordResourceGains(collectedResources, collectedResourceSources, cardId, true, collectAndClearCardStateToPlayer(ctx.G, ctx.playerId, cardId)));
      ...
      createReactiveExhaustChoicesForResourceGainSources(ctx, collectedResourceSources, collectedResources);
      if (pendingEffectInterruption(ctx.G)) {
        appendResumeEffectsToPending(ctx.G, effect.effects);
        return true;
      }
      return runEffects(ctx, effect.effects);
    }
    case "garrison_card": {
      if (!effect.hostCardId || !effect.cardId) {
        ...
        if (hostCardIds.length === 1 && cardIds.length === 1) {
          if (!garrisonCardOnRegion(ctx.G, ctx.playerId, hostCardIds[0], cardIds[0])) {
            ctx.G.log.push({ round: ctx.G.round, playerId: ctx.playerId, message: `GarrisonFailed(${cardIds[0]}/host=${hostCardIds[0]})` });
            return false;
          }
          break;
        }
    ...
    case "develop": {
      return createCardDrivenDevelopmentChoice(ctx.G, ctx.playerId, effect.sourceCardId ?? ctx.selfCardId, { free: effect.free });
    }
    case "move_self_to_history": {
      ...
      if (garrisonHostCardId) {
        recordResourceGains(collectedResources, collectedResourceSources, ctx.selfCardId, true, collectAndClearCardStateToPlayer(ctx.G, ctx.playerId, ctx.selfCardId));
        movedCardIds = [ctx.selfCardId];
      } else {
        removeOneCard(p.playArea, ctx.selfCardId);
        removeOneCard(p.powerArea, ctx.selfCardId);
    ...
    case "exile_card": {
      if (isPlayerExileSource(effect.source)) {
        ...
        const cardIds = playerExileSourceCards(ctx.G, ctx.playerId, effect.source).filter((sourceCardId) => cardMatchesExileCriteria(ctx.G, ctx.playerId, sourceCardId, effect, effect.source));
      ...
      const cardIds = ctx.G.market.filter((marketCardId) => cardMatchesExileCriteria(ctx.G, ctx.playerId, marketCardId, effect, "market"));
      if (cardIds.length === 0) break;
      const resumeEffects = remainingExileEffect(effect, 1);
      ...
      for (let i = 0; i < effect.count; i++) {
        if (effect.source === "exile") {
          const cardId = effect.cardId;
            if (!cardId) {
              const cardIds = availableExileCards(ctx.G, ctx.playerId).filter((exiledCardId) =>
                canAcquireExileCard(ctx.G, exiledCardId) && cardMatchesAcquireCriteria(ctx.G, ctx.playerId, exiledCardId, effect)
              );
              if (cardIds.length === 0) break;
              if (cardIds.length === 1) {
                const takenUnrestPlayerIds: string[] = [];
                if (!acquireFromExile(ctx.G, { playerId: ctx.playerId, cardId: cardIds[0], destination: effect.destination, takenUnrestPlayerIds, randomNumber: ctx.randomNumber })) {
                  if (ctx.G.gameover) break;
                  return false;
                }
                if (pendingEffectInterruption(ctx.G)) {
                  ctx.G.pendingAcquireEffectResolution = {
                    ...
                    sourceCardId: ctx.selfCardId,
                    takenUnrestPlayerIds,
                    resumeEffects: remainingAcquireEffect(effect, i + 1)
                  };
                  break;
                }
                if (!runAcquireTriggers(ctx, cardIds[0])) return false;
                if (!runEffectNationHooks(ctx, "after_acquire", [cardIds[0]])) return false;
                if (pendingEffectInterruption(ctx.G)) {
          ...
          const takenUnrestPlayerIds: string[] = [];
          if (!acquireFromExile(ctx.G, { playerId: ctx.playerId, cardId, destination: effect.destination, takenUnrestPlayerIds, randomNumber: ctx.randomNumber })) {
            if (ctx.G.gameover) break;
            return false;
          }
          if (pendingEffectInterruption(ctx.G)) {
          ...
          if (!runEffectNationHooks(ctx, "after_acquire", [cardId])) return false;
          if (pendingEffectInterruption(ctx.G)) {
            appendResumeEffectsToPending(ctx.G, remainingAcquireEffect(effect, i + 1));
            break;
          }
        ...
        if (effect.cardId) {
          ...
          const collectedResourceSources: ResourceGainSource[] = [];
          const takenUnrestPlayerIds: string[] = [];
          if (!acquireMarketCard(ctx.G, { playerId: ctx.playerId, cardId: effect.cardId, destination: effect.destination ?? "hand", collectedResources, collectedResourceSources, takenUnrestPlayerIds, randomNumber: ctx.randomNumber })) {
            if (ctx.G.gameover) break;
            return false;
          }
          if (pendingEffectInterruption(ctx.G)) {
          ...
          if (!runEffectNationHooks(ctx, "after_acquire", [effect.cardId])) return false;
          if (pendingEffectInterruption(ctx.G)) {
            appendResumeEffectsToPending(ctx.G, remainingAcquireEffect(effect, i + 1));
            break;
          }
          if (runAcquireReactiveWindows(ctx, effect.cardId, takenUnrestPlayerIds, collectedResources, collectedResourceSources, remainingAcquireEffect(effect, i + 1))) break;
          continue;
        }
        if (!effect.suit && !effect.cardType) {
          ...
          const collectedResourceSources: ResourceGainSource[] = [];
          const takenUnrestPlayerIds: string[] = [];
          if (!acquireMarketCard(ctx.G, { playerId: ctx.playerId, cardId: cardIds[0], destination: effect.destination ?? "hand", collectedResources, collectedResourceSources, takenUnrestPlayerIds, randomNumber: ctx.randomNumber })) {
            if (ctx.G.gameover) break;
            return false;
          }
          if (pendingEffectInterruption(ctx.G)) {
          ...
          if (runAcquireReactiveWindows(ctx, cardIds[0], takenUnrestPlayerIds, collectedResources, collectedResourceSources, remainingAcquireEffect(effect, i + 1))) break;
          continue;
        }
        const cardIds = ctx.G.market.filter((marketCardId) => cardMatchesAcquireCriteria(ctx.G, ctx.playerId, marketCardId, effect));
        if (cardIds.length === 0) break;
        if (cardIds.length === 1) {
          const collectedResources: Partial<Record<ResourceName, number>> = {};
          const collectedResourceSources: ResourceGainSource[] = [];
    ...
    case "take_card": {
      ...
      for (let i = 0; i < effect.count; i += 1) {
        const cardIds = effect.cardId && i === 0
          ? matchingMarketMoveCards(ctx.G, ctx.playerId, effect).filter((cardId) => cardId === effect.cardId)
          : matchingMarketMoveCards(ctx.G, ctx.playerId, { ...effect, cardId: undefined });
        if (cardIds.length === 0) break;
        if (cardIds.length > 1) {
        ...
        const moved = effect.op === "gain_card"
          ? gainMarketCard(ctx.G, { playerId: ctx.playerId, cardId: cardIds[0], destination: effect.destination ?? "hand", collectedResources, collectedResourceSources, takenUnrestPlayerIds, randomNumber: ctx.randomNumber })
          : takeMarketCard(ctx.G, { playerId: ctx.playerId, cardId: cardIds[0], destination: effect.destination ?? "hand", collectedResources, collectedResourceSources });
        if (!moved) {
          if (ctx.G.gameover) break;
          return false;
        }
        ...
        if (pendingEffectInterruption(ctx.G)) {
          ctx.G.pendingMarketMoveEffectResolution = {
            ...
            collectedResources,
            collectedResourceSources,
            resumeEffects: remainingMarketMoveEffect(effect, i + 1)
          };
          break;
        }
        if (runMarketMoveReactiveWindows(ctx, takenUnrestPlayerIds, collectedResources, collectedResourceSources, remainingMarketMoveEffect(effect, i + 1))) break;
      }
      break;
    }
    case "break_through": {
      if (effect.source === "market") {
        for (let completedCount = 0; completedCount < effect.count; completedCount += 1) {
          const cardIds = effect.cardId && completedCount === 0
            ? matchingMarketBreakThroughCards(ctx.G, ctx.playerId, effect).filter((cardId) => cardId === effect.cardId)
            : matchingMarketBreakThroughCards(ctx.G, ctx.playerId, { ...effect, cardId: undefined });
          if (cardIds.length === 0) break;
          if (cardIds.length > 1) {
          ...
          const result = breakThrough(ctx.G, { playerId: ctx.playerId, suit: effect.suit, source: "market", count: 1, cardId: cardIds[0], randomNumber: ctx.randomNumber });
          if (!ctx.G.gameover) createReactiveExhaustChoicesForResourceGainSources(ctx, result.marketResourceGainSources, result.marketResourceGains);
          if (!ctx.G.gameover && pendingEffectInterruption(ctx.G)) {
            ctx.G.pendingBreakThroughEffectResolution = {
              playerId: ctx.playerId,
              sourceCardId: ctx.selfCardId,
              gainedCardIds: result.gainedCardIds,
              resumeEffects: remainingBreakThroughEffect(effect, completedCount + 1)
            };
            break;
          }
          if (!ctx.G.gameover && !pendingEffectInterruption(ctx.G) && result.gainedCardIds.length > 0) {
            createReactiveExhaustChoice(ctx, { trigger: "after_break_through_card", targetPlayerId: ctx.playerId });
            if (pendingEffectInterruption(ctx.G)) {
              ctx.G.pendingBreakThroughEffectResolution = {
                playerId: ctx.playerId,
                sourceCardId: ctx.selfCardId,
      ...
      if (effect.source === "exile" && !effect.cardId) {
        const cardIds = availableExileCards(ctx.G, ctx.playerId).filter((exiledCardId) => cardMatchesBreakThroughCriteria(ctx.G, ctx.playerId, exiledCardId, effect));
        if (cardIds.length === 0) break;
        if (cardIds.length === 1) {
          const result = breakThrough(ctx.G, { playerId: ctx.playerId, suit: effect.suit, source: "exile", count: 1, cardId: cardIds[0], randomNumber: ctx.randomNumber });
          if (!ctx.G.gameover && !pendingEffectInterruption(ctx.G) && result.gainedCardIds.length > 0) {
            createReactiveExhaustChoice(ctx, { trigger: "after_break_through_card", targetPlayerId: ctx.playerId });
            if (pendingEffectInterruption(ctx.G)) {
              ctx.G.pendingBreakThroughEffectResolution = {
                ...
                gainedCardIds: result.gainedCardIds,
                nextAfterBreakThroughReactiveCardIndex: 1,
                resumeEffects: remainingBreakThroughEffect(effect, 1)
              };
              break;
            }
          }
          if (!ctx.G.gameover && !pendingEffectInterruption(ctx.G) && result.gainedCardIds.length > 0) {
            if (!runBreakThroughNationHooks(ctx, result.gainedCardIds, remainingBreakThroughEffect(effect, 1))) return false;
          }
          break;
        }
      ...
      if (effect.source === "deck" && effect.suit === "tributary" && !effect.cardId) {
        const cardIds = visibleTributaryBreakThroughCards(ctx.G, ctx.playerId);
        if (cardIds.length > 1) {
          const resumeEffects = remainingBreakThroughEffect(effect, 1);
          ctx.G.pendingBreakThroughChoice = {
            playerId: ctx.playerId,
      ...
      const result = breakThrough(ctx.G, { playerId: ctx.playerId, suit: effect.suit, source: effect.source, count: effect.count, cardId: effect.cardId, randomNumber: ctx.randomNumber });
      if (!ctx.G.gameover) createReactiveExhaustChoicesForResourceGainSources(ctx, result.marketResourceGainSources, result.marketResourceGains);
      if (!ctx.G.gameover && pendingEffectInterruption(ctx.G)) {
        ctx.G.pendingBreakThroughEffectResolution = {
          playerId: ctx.playerId,
    ...
    case "choose_one": {
      ...
      if (choices.length === 1 && effect.choices.length > 1) return runEffects(ctx, choices[0]);
      ctx.G.pendingChoice = { playerId: ctx.playerId, sourceCardId: ctx.selfCardId, choices };
