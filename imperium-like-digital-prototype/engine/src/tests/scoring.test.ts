...
describe("scoring", () => {
  it("scores owned cards in normal scoring zones, including garrisoned cards", () => {
    ...
    G.players["1"].hand = ["hand_vp"];
    G.players["1"].deck = ["deck_vp"];
    G.players["1"].discard = ["discard_vp"];
    G.players["1"].playArea = ["play_vp"];
    G.players["1"].history = ["history_vp"];
    G.players["1"].powerArea = ["power_vp"];
    ...
    expect(scorePlayer(G, "1")).toBe(37);
  });
  ...
  it("does not score unplayed Nation deck or undeveloped Development area cards", () => {
    ...
    G.players["1"].nationDeck = ["nation_vp"];
    G.players["1"].developmentArea = ["development_vp"];
    G.players["1"].discard = ["discard_vp"];

    expect(scorePlayer(G, "1")).toBe(4);
  });
  ...
  it("does not score garrisoned cards attached to unplayed Nation or Development cards", () => {
    ...
    G.players["1"].nationDeck = ["nation_host"];
    G.players["1"].developmentArea = ["development_host"];
    G.players["1"].discard = ["discard_vp"];
    G.cardStates = {
  ...
  it("scores only Progress resources from the player resource pool", () => {
    const G = createInitialState();
    G.players["1"].resources.materials = 5;
    G.players["1"].resources.knowledge = 4;
    G.players["1"].resources.influence = 7;
    G.players["1"].resources.goods = 3;
  ...
  it("can score Progress resources at a reduced state-gated nation ratio", () => {
    const G = createInitialState();
    G.players["1"].resources.knowledge = 8;
  ...
  it("caps variable victory points at 10 and applies structured VP modes", () => {
    ...
    G.players["1"].discard = ["fixed_vp", "variable_vp", "negative_vp", "none_vp", "conditional_vp"];
  ...
  it("scores imported numeric conditional victory points and ignores unresolved conditionals", () => {
    ...
    G.players["1"].discard = ["conditional_met", "conditional_unresolved"];
  ...
  it("scores structured conditional victory points based on the card scoring zone", () => {
    ...
    G.players["1"].history = ["conditional_history"];
    G.players["1"].discard = ["conditional_discard"];

    expect(scorePlayer(G, "1")).toBe(11);
  });
  ...
  it("scores structured variable victory points by counting matching cards in scored zones with a cap", () => {
    ...
    G.players["1"].discard = ["variable_counter"];
    G.players["1"].playArea = ["play_region_a", "play_region_b"];
    G.players["1"].history = ["history_region"];
    G.players["1"].deck = ["deck_region_not_counted"];
  ...
  it("treats missing imported card tags as non-matches for structured variable victory points", () => {
    ...
    G.players["1"].discard = ["variable_counter"];
    G.players["1"].playArea = ["untagged_region", "tagged_region"];
  ...
  it("does not count Trade Routes as in-play cards for structured variable victory points", () => {
    ...
    G.players["1"].discard = ["variable_counter"];
    G.players["1"].playArea = ["play_region", "route_with_region_tag"];
  ...
  it("does not count imported Trade Route suit icons as in-play cards for structured variable victory points", () => {
    ...
    G.players["1"].discard = ["variable_counter"];
    G.players["1"].playArea = ["play_region", "imported_route_with_region_tag"];
  ...
  it("scores structured variable victory points from resource pools without counting resources on cards", () => {
    ...
    G.players["1"].discard = ["resource_counter"];
    G.players["1"].playArea = ["resource_host"];
  ...
  it("scores structured variable victory points from card resources when formula names scored zones", () => {
    ...
    G.players["1"].discard = ["resource_counter", "discard_host"];
    G.players["1"].playArea = ["play_host"];
  ...
  it("treats scored History replacement zones as History for structured VP rules", () => {
    ...
    G.players["1"].discard = ["variable_counter"];
    G.players["1"].history = [];
    G.players["1"].sideAreas = { sunken: ["conditional_history", "sunken_region"] };
  ...
  it("applies History scoring exclusions to structured variable victory point formulas", () => {
    ...
    G.players["1"].hand = ["variable_counter"];
    G.players["1"].playArea = ["play_vp"];
    G.players["1"].history = [];
    G.players["1"].sideAreas = { sunken: ["sunken_region"] };
    G.activeNationRulesets = {
      ...G.activeNationRulesets,
      "1": {
        ...G.activeNationRulesets!["1"],
  ...
  it("honors scoring zone exclusions", () => {
    ...
    G.players["1"].discard = ["discard_vp"];
    G.players["1"].history = ["history_vp"];
    G.activeNationRulesets!["1"].scoringOverrides = [{ op: "exclude_zone_from_scoring", zoneId: "discard" }];
  ...
  it("applies History scoring exclusions to scored History replacement zones", () => {
    ...
    G.players["1"].discard = ["discard_vp"];
    G.players["1"].history = [];
    G.players["1"].sideAreas = { sunken: ["sunken_vp"] };
  ...
  it("normal scoring waits for the current round, one final round, and final solstice", () => {
    ...
    G.players["1"].powerArea = ["p0_vp"];
    G.players["2"].powerArea = ["p1_vp"];
    const playOrder = ["1", "2"];

    triggerScoring(G, "test_trigger", "1");

    expect(G.scoring).toEqual({ reason: "test_trigger", triggeredBy: "1", phase: "finish_current_round" });
    ...
    onTurnEnd(G, { currentPlayer: "2", playOrder } as any);
    expect(G.round).toBe(2);
    expect(G.gameover).toBeUndefined();
    expect(G.scoring).toEqual({ reason: "test_trigger", triggeredBy: "1", phase: "final_round", finalRound: 2 });
  ...
  it("clears the active scoring lifecycle when normal scoring finalizes", () => {
    ...
    G.players["1"].hand = ["p0_vp"];
    G.players["2"].hand = ["p1_vp"];
    G.scoring = { reason: "test_trigger", triggeredBy: "1", phase: "final_round", finalRound: 1 };
  ...
  it("finalizes normal scoring when imported rulesets omit lifecycle arrays", () => {
    ...
    G.scoring = { reason: "minimal_ruleset", triggeredBy: "1", phase: "final_round", finalRound: 1 };
  ...
  it("solo normal scoring compares the human score to the Bot score and ties go to the Bot", () => {
    ...
    G.players["1"].hand = ["human_vp_a", "human_vp_b"];
    ...
    G.scoring = { reason: "solo_test", triggeredBy: "1", phase: "final_round", finalRound: 1 };
  ...
  it("records a campaign outcome snapshot when solo campaign scoring finishes", () => {
    ...
    G.players["1"].hand = ["human_vp"];
    G.solo!.bot.botNationId = "campaign_bot";
    G.solo!.bot.botDeck = ["bot_vp"];
    G.scoring = { reason: "campaign_test", triggeredBy: "1", phase: "final_round", finalRound: 1 };
  ...
  it("keeps multiplayer scoring finalization when later option mutations switch to solo", () => {
    ...
    triggerScoring(G, "multiplayer_test", "1");
    G.scoring = { ...G.scoring!, phase: "final_round", finalRound: 1 };
  ...
  it("collapse scoring ends immediately and uses lowest Unrest instead of VP", () => {
    ...
    G.players["1"].discard = ["p0_vp", "p0_unrest"];
    G.players["2"].discard = ["p1_vp", "unrest_card", "p1_unrest_extra_a", "p1_unrest_extra_b"];

    triggerCollapse(G, "unrest_pile_empty", "2");
    ...
    expect(G.scoring).toBeUndefined();
    expect(G.log.at(-1)?.message).toBe("CollapseFinalized(winner=1)");
  });

  it("collapse scoring ignores resource-pool Unrest counters and counts only Unrest cards in scoring zones", () => {
    const G = createInitialState();
    G.players["1"].resources.unrest = 9;
    G.players["2"].resources.unrest = 0;
  ...
  it("collapse scoring counts imported cards with an Unrest suit icon as Unrest", () => {
    ...
    G.players["2"].powerArea = ["p1_vp"];
    G.players["2"].discard = ["imported_unrest"];
  ...
  it("collapse scoring counts Unrest in nation-specific History replacement zones", () => {
    ...
    G.players["1"].sideAreas = { flooded: ["flooded_unrest"] };
    G.activeNationRulesets!["1"].zoneOverrides = [
  ...
  it("collapse scoring does not count Unrest cards in the Power area", () => {
    ...
    G.players["1"].powerArea = ["power_unrest"];
  ...
  it("collapse scoring does not count garrisoned Unrest attached to ignored zones", () => {
    ...
    G.players["1"].nationDeck = ["nation_host"];
    G.players["1"].developmentArea = ["development_host"];
    G.players["1"].exile = ["exile_host"];
    G.players["1"].discard = ["counted_host"];
    G.cardStates = {
  ...
  it("collapse tie among lowest-Unrest players is broken by normal scoring among tied players", () => {
    ...
    G.players["3"] = { ...G.players["1"], resources: { ...G.players["1"].resources, unrest: 3, influence: 0 }, discard: [], hand: [], deck: [], playArea: [], history: [], exile: [], powerArea: ["p2_vp"], stateArea: [], developmentArea: [], nationDeck: [] };
    G.players["1"].powerArea = ["p0_vp"];
    G.players["2"].powerArea = ["p1_vp"];
    G.players["1"].discard = ["p0_unrest"];
    G.players["2"].discard = ["p1_unrest"];
    G.players["3"].discard = ["p2_unrest_a", "p2_unrest_b", "p2_unrest_c"];
  ...
  it("collapse tie-break scoring allows tied players to return Unrest from scoring zones without changing the original Unrest tie", () => {
    ...
    G.players["1"].powerArea = ["p0_vp"];
    G.players["1"].deck = ["p0_unrest"];
    G.players["2"].hand = ["p1_unrest"];
    G.activeNationRulesets!["1"] = {
      ...G.activeNationRulesets!["1"],
      hookRules: [{
        trigger: "before_scoring",
  ...
  it("collapse tie-break scoring can return Unrest from a nation-specific History replacement zone", () => {
    ...
    expect(G.players["1"].sideAreas?.flooded).toEqual([]);
  ...
  it("routes injected randomness through scoring lifecycle hooks", () => {
    ...
    expect(G.players["1"].hand).toEqual(["random_keep"]);
  ...
  it("pauses normal scoring finalization when before_scoring opens a reactive Exhaust window", () => {
    ...
    resolveReactiveExhaustChoice({ G, ctx: { currentPlayer: "1" } as any }, exhaustCardId);
  ...
  it("pauses normal scoring finalization when before_scoring creates a pending Exile choice", () => {
    ...
    resolveExileChoice({ G, ctx: { currentPlayer: "1" } as any }, "market_civilized");
  ...
  it("pauses normal scoring finalization when before_scoring creates a pending Return Unrest choice", () => {
    ...
    G.players["1"].hand = ["p0_vp", "hand_unrest", "hand_unrest_b"];
    G.players["2"].hand = ["p1_vp"];
    G.scoring = { reason: "test_trigger", triggeredBy: "1", phase: "final_round", finalRound: 1 };
    G.activeNationRulesets!["1"] = {
      ...G.activeNationRulesets!["1"],
    ...
    resolveReturnUnrestChoice({ G, ctx: { currentPlayer: "1" } as any }, "hand_unrest");
  ...
  it("normal scoring return-Unrest choices include nation-specific History replacement zones", () => {
    ...
    resolveReturnUnrestChoice({ G, ctx: { currentPlayer: "1" } as any }, "flooded_unrest");
  ...
  it("normal scoring return-Unrest choices include garrisoned Unrest attached to scored-zone hosts", () => {
    ...
    G.players["1"].powerArea = ["p0_vp"];
    G.players["1"].hand = ["hand_unrest"];
    G.players["1"].discard = ["discard_host"];
    G.players["2"].powerArea = ["p1_vp"];
    ...
    resolveReturnUnrestChoice({ G, ctx: { currentPlayer: "1" } as any }, "garrisoned_unrest");

    expect(G.pendingReturnUnrestChoice).toBeUndefined();
    expect(G.players["1"].discard).toEqual(["discard_host"]);
  ...
  it("pauses normal scoring finalization when before_scoring creates newer pending keyword choices", () => {
    const scenarios = [
      {
        ...
        setup(G: ReturnType<typeof createInitialState>) {
          G.players["1"].hand = ["p0_vp", "place_target"];
          G.cardDb.place_target = vpCard("place_target", 0);
        },
        resolve(G: ReturnType<typeof createInitialState>) {
          resolvePlaceOnDeckChoice({ G, ctx: { currentPlayer: "1" } as any }, "place_target");
        }
      },
      {
        ...
        setup(G: ReturnType<typeof createInitialState>) {
          G.players["1"].hand = ["p0_vp", "give_target"];
          G.cardDb.give_target = vpCard("give_target", 0);
        },
        resolve(G: ReturnType<typeof createInitialState>) {
          resolveGiveCardChoice({ G, ctx: { currentPlayer: "1" } as any }, "give_target", "2");
        }
      },
      {
        ...
        setup(G: ReturnType<typeof createInitialState>) {
          G.players["1"].hand = ["p0_vp", "hand_civilized", "hand_uncivilized"];
        ...
        resolve(G: ReturnType<typeof createInitialState>) {
          resolveSwapChoice({ G, ctx: { currentPlayer: "1" } as any }, "hand_civilized", "market_civilized");
        }
      },
      {
        ...
        resolve(G: ReturnType<typeof createInitialState>) {
          resolveMarketCardChoice({ G, ctx: { currentPlayer: "1" } as any }, "market_civilized_b");
        }
      }
    ] as const;
    ...
    for (const scenario of scenarios) {
      ...
      G.players["1"].hand = ["p0_vp"];
      G.players["2"].hand = ["p1_vp"];
      G.scoring = { reason: scenario.expectedKey, triggeredBy: "1", phase: "final_round", finalRound: 1 };
      scenario.setup(G);
      G.activeNationRulesets!["1"] = {
        ...G.activeNationRulesets!["1"],
        hookRules: [{
  ...
  it.each([
    {
      ...
      setPending(G: ReturnType<typeof createInitialState>) {
        G.pendingCleanupMarketResourceChoice = { playerId: "1", resource: "knowledge", amount: 1, cardIds: ["market_card"] };
      }
    },
    {
    ...
    G.players["1"].hand = ["p0_vp", "discard_option"];
    G.players["2"].hand = ["p1_vp"];
    G.scoring = { reason: "cleanup_pause", triggeredBy: "1", phase: "final_round", finalRound: 1 };
