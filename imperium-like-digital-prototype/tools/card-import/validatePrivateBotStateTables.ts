...
function validateEffectSuitFilter(errors: BotStateTableImportError[], row: number, value: unknown, path: string): boolean {
...
function validateEffectNumericPayload(errors: BotStateTableImportError[], row: number, record: Record<string, unknown>, path: string): boolean {
...
function validateEffectRequiredPayloads(errors: BotStateTableImportError[], row: number, record: Record<string, unknown>, path: string): boolean {
  const op = String(record.op);
  let fatal = false;
...
function validateEffects(errors: BotStateTableImportError[], row: number, effects: unknown, path = "effects_json", validateResources = true): boolean {
  ...
  effects.forEach((effect, index) => {
    ...
    fatal = validateEffectSuitFilter(errors, row, filter, `${path}[${index}].filter`) || fatal;
    fatal = validateEffectSuitFilter(errors, row, marketFilter, `${path}[${index}].marketFilter`) || fatal;
    fatal = validateEffectNumericPayload(errors, row, effect as Record<string, unknown>, `${path}[${index}]`) || fatal;
    fatal = validateEffectRequiredPayloads(errors, row, effect as Record<string, unknown>, `${path}[${index}]`) || fatal;
    if (op === "bot_pay_resource_then") {
      fatal = validateEffects(errors, row, (effect as { effects?: unknown }).effects, `${path}[${index}].effects`, false) || fatal;
    }
    ...
    if (ifVp !== undefined) {
      if (!ifVp || typeof ifVp !== "object" || Array.isArray(ifVp)) {
        ...
        fatal = validateEffects(errors, row, condition.effects, `${path}[${index}].ifVp.effects`, false) || fatal;
      }
    }
  });
  return fatal;
