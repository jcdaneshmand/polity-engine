import type { EffectOp, ExpansionId } from "./cardCsvTypes";
...
export type NationDefinition = { id:string; displayName:string; privateName?:string; sourceBox?:string; complexity?:number; powerCardIds:string[]; stateCardIds:string[]; startingDeckCardIds:string[]; nationDeckCardIds:string[]; accessionCardId?:string; developmentCardIds:string[]; setupRules:SetupRule[]; passiveRules:NationRuleHook[]; actionTokensBase:number; exhaustTokensBase:number; requiredExpansions:ExpansionId[]; excludedExpansions?: ExpansionId[]; allowedModes?: ("multiplayer"|"solo"|"practice")[]; disallowedModes?: ("multiplayer"|"solo"|"practice")[]; notes?:string; implemented:boolean; tested:boolean; };
