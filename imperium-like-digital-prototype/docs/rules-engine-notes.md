...
- Runtime `Suit`: `region | uncivilized | civilized | tributary | fame | unrest | power | trade_route | none | multi`, with importer/validator aliases handling rulebook spelling variants such as Uncivilised/Civilised where private data uses them.
- Runtime `CardType`: `action | unit | technology | legacy | in_play | attack | power | state | development | accession | nation | region | unrest | fame | trade_route | bot_state | other`.
...
- Runtime `ZoneName`: `hand | drawDeck | discard | playArea | history | nationDeck | developmentArea | garrison | exile | market | unrestPile | fameDeck | mainDeck | regionDeck | uncivilizedDeck | civilizedDeck | tradeRouteDeck | botDeck | botDiscard | botSlots | removedFromGame`.
- Runtime `Resource`: `materials | influence | knowledge | goods | unrest`, with importer normalization from rulebook-facing Population/Progress labels into the engine keys.
...
- Track market slots as structured objects: visible card ID, suit/source deck, resources on the slot card, unrest tucked under the card, and refill metadata. Runtime market-resource changes must keep the structured slot markers in sync with the legacy card-id resource map, and Market token/attached-Unrest collection must read either representation without double-counting synced copies.
...
- History is owner-visible: cards placed into History score normally and generally cannot be used again, and their owner may always inspect them, but other players may not see their identities. Opponents should see only the History count unless a card or mode explicitly reveals more. Discard, play area, exile, and garrison contents are public unless a nation-specific rule says otherwise.
- Nation-specific side areas and History replacement zones must appear in player zone chrome with their current counts and data-driven display names, while still applying their visibility policy in detailed inspection.
- Regular Nation deck cards are shuffled during setup and then hidden; Accession is the public bottom card; Development choice is face-up and player-selected. If Accession is stored separately in runtime state, Nation deck counts, progression checks, ruleset zone conditions, and collapse/auto-win zone checks must still treat it as the bottom Nation deck card while UI inspection exposes only that Accession identity, not the hidden regular Nation cards. If Accession is stored inside `nationDeck`, UI inspection exposes it only while it is still the bottom card.
...
2. For each player, configure Power card, State card, starting deck, starting resources, Nation deck, Development area, hand size, action/exhaust tokens, and any nation-specific setup rules. The default starting resource pool is 3 Materials, 2 Population, and 1 Progress; with Trade Routes enabled, the 1 Progress is replaced by 1 Goods.
   - A single physical two-sided State card must still have an active side in runtime state. Default nations start on the Barbarian/uncivilized side unless a nation ruleset explicitly starts on another side. With Trade Routes enabled, each player's extra Exhaust token is an expansion modifier on top of the active State card's printed/default Exhaust count, including after state flips or other state-stat refreshes.
3. Regular Nation deck cards are shuffled into a hidden Nation deck, with Accession tracked separately as the public bottom card underneath them. The current top card of that shuffled Nation deck is the next progression card that will be added during reshuffle.
...
- Draw-if-able effects never trigger reshuffle. They stop when the draw deck is empty. If Draw-if-able text is voluntary up to a count, the player may choose zero through the currently drawable deck count before later effect text resumes.
...
1. If the player has cards in their Nation deck, take the next/top Nation card and add it to the discard before reshuffling. If this was the accession/state-changing card, flip or change the State card at the correct point. Move an available Action token from the State card onto the Nation deck for that reshuffle marker, or into the Development area if that addition empties the Nation deck.
2. If the player has no Nation deck cards left and has at least one card in Development area, they may pay the development cost of one face-up Development card they can afford. The chosen card goes to discard. Move an available Action token from the State card into the Development area. If no remaining Development card is payable, no Develop occurs.
...
- Nation/Development reshuffle progression requires an available Action token and no existing reshuffle progression marker on the Nation deck or Development area; otherwise skip the progression addition and only shuffle discard.
...
- The Unrest tucked under an acquired market card is taken into the player's hand unless the acquisition rule says otherwise, reading either the legacy Market-Unrest map or structured Market slot attachment. Region cards should not have tucked Unrest.
...
- One path takes a matching face-up market card into hand, gains resources on it, and returns its tucked Unrest instead of taking it, reading either the legacy Market-Unrest map or structured Market slot attachment. Those gained market-resource tokens can open resource-gain reactive Exhaust windows before later effect text resumes. If more than one market card matches, the choice must be explicit or otherwise deterministic by the relevant solo/bot tie-break rule.
...
- Find for an exact named card searches hand, discard, draw deck, then Nation deck while excluding the accession card and stops at the first found copy; if the hit is in the draw or Nation deck, move/reveal that found card first, then shuffle the remaining searched deck afterward. Find by criteria, such as "Find a Civilised card", searches all listed sources before presenting the active player with every eligible choice, but any searched draw/Nation deck shuffle is deferred until after the chosen card is moved so the chosen card is not shuffled back into its source. Effects may explicitly restrict the searched sources or include play area, History, or garrison; those non-default sources should be searched only when specified. A Find from garrison targets the attached child card itself, collects any resources on that child, and leaves its host in play. A Find that moves a play-area host carries its garrisoned cards and uses normal hosted-resource collection and reactive timing.
- Look effects reveal the requested cards to the looking player without moving them. If fewer eligible cards exist, reveal as many as possible. Look-only action text is playable only when the requested source has at least one eligible card to inspect. When looking at multiple cards from the same hidden deck, the player may return the looked cards in any order. Look-and-take effects are distinct from Find: they reveal only the eligible top cards, let the player take one of those cards to the specified destination, return the remaining looked cards in chosen order, and resume later text afterward. When looking at the Nation deck, ignore the accession card unless it is the only card available; this applies whether the engine stores the Accession card inside `nationDeck` or separately as `accessionCardId`. When looking at Fame, ignore the special bottom Fame card while ordinary Fame cards remain, but reveal it if it is the only face-up Fame card left.
...
- Swap exchanges an eligible player-owned card, usually from hand, with a matching-suited market card. "Matching-suited" means the Market card has at least one suit icon also present on the swapping card; card import should preserve additional printed suit icons as explicit suit metadata, currently represented by `suit:<name>` tags when one primary `suit` is not enough. Resources on the market card stay with the market slot and transfer onto the incoming card, reading from either the legacy Market-resource map or structured Market slot markers without double-counting synced copies; Unrest tucked under the outgoing market card returns to the Unrest pile from either legacy or structured slot state; a fresh Unrest is tucked under the incoming card unless that card is ineligible such as Unrest or Region. The outgoing market card moves to the source zone the swapping card came from, and multi-option swaps interrupt the effect list until the player chooses the exact pair.
...
- Payment conversion follows the rulebook hierarchy: when paying Materials, Progress and Goods may each be spent as 2 Materials with no change; when paying Population, Progress and Goods may each be spent as 1 Population; Progress and Goods costs must be paid directly. Materials and Population do not convert into each other by default. Runtime player resource pools are canonicalized before affordability and spend checks, so rulebook-named Progress/Population keys do not make an otherwise payable cost appear unpaid.
...
- Printed costs that allow discarding hand cards are represented as selected discard-card costs, not random discard effects. They can interrupt effect resolution and resume the remaining effects after the chosen cards move to discard. Imported discard-card costs may also restrict eligible hand cards by suit icon and/or card type when the printed cost names a card category.
- Some card text moves resources from a player's pool onto public Market cards instead of paying them to supply. This is a distinct resource movement: the chosen resources leave the player's pool, are placed on distinct eligible Market cards, mirror into both legacy Market-resource maps and structured Market slot markers, and later effect text waits behind any required destination choice.
...
- Runtime resource aliases are canonicalized before after-gain-resource reactive Exhaust matching and pending-choice resolution, so printed Progress/Population text matches engine Knowledge/Influence listeners. Resource amount reads also canonicalize the stored resource bag, so canonical effect text can still see and update a runtime pool that was restored or imported with rulebook-named Progress/Population keys.
...
- Any effect-created player choice interrupts the current effect list: effects after the choice do not resolve until the player resolves the pending choice, then the remaining effects resume in order. This applies to choose-one/optional choices and keyword choices such as Draw from a face-up zone, Find, Acquire, Gain/Take from the Market, Break through, Exile, Garrison, Recall/Abandon, Develop, Trade, and Return Exhaust token, including choices created during reshuffle hooks, Solstice, and scoring hooks. Counted Region-choice, Acquire-effect, Market-move, Break-through post-gain reactive/passive-hook continuations, and pending scoring finalization are also effect-resolution interruptions: unrelated new effects must wait, while the owned choice/reactive Exhaust/passive continuation may finish its queued sequence.
- Dynamic player groups use public-safe `targetPlayerScope` values. `self` targets only the resolving player, `all` targets every current player, and `others` targets every current player except the resolver. Current runtime support covers Draw, gain-resource, Steal-resource, Recall/Abandon Region, and Take-Unrest effects, with explicit fixed-recipient arrays where the rule text names specific players. Draw supports per-target optional choices; Steal supports scoped targets plus per-target fallback effects when a target cannot pay; targeted Region effects create target-owned choices and resume later text under the original resolver. Targeted Take-Unrest attack effects can be marked as targeted attacks; protected targets ignore that effect when they have a player protection flag or a visible persistent protection tag.
...
- History is an owner-visible scored zone. Cards in History are normally out of future play but still count for scoring unless a specific rule says otherwise; opponents may not inspect those card identities.
...
- A player with multiple simultaneous effects chooses their own resolution order when those effects can interact, mutate shared state, or create pending choices such as Exile, Return Unrest, place-on-deck, Give, Swap, Return Exhaust token, Gain/Take card, or optional/choose-one choices; independent resource gains can resolve automatically.
- Deck-manipulating Solstice effects such as Look, Draw, Draw-if-able, Fame gain, place-on-deck, and Market/Exile Gain/Take-card effects are order-sensitive when multiple Solstice cards are resolving for the same player, because the chosen order can change hidden deck order, pending choices, and scoring triggers.
...
- Score Progress tokens in the player's resource pool at 1 victory point each. Runtime resource aliases such as Progress/Knowledge are canonicalized before the player resource pool is scored. Materials, Population, Goods, and Unrest do not inherently score as resource-pool tokens. Tokens on cards do not score as resource-pool tokens unless a card or nation rule explicitly says otherwise.
- Nation rules may replace resource-pool scoring with a state-gated ratio, such as Alien Martians scoring Progress at 1 victory point per 3 Progress. Runtime resource aliases in those scoring overrides are canonicalized before the override is matched.
- Positive VP from each scored card is capped at the rulebook maximum unless a card/rule overrides it.
- Structured variable VP formulas currently support counting cards by tag or suit in selected scoring zones, plus counting selected player resource pools with an optional per-resource-group denominator and cap. Resource-counting VP uses the player's resource pool by default, and can explicitly count resources stored on cards in named scored zones when a card or nation rule calls for that. Runtime resource aliases are canonicalized before resource-counting VP formulas are evaluated, including aliases stored on card resource state.
...
- In Bot scoring, each Progress scores 1 VP. Below Sovereign, score 1 VP per 10 Materials/Population with each Goods counting as 5 toward that total. On Sovereign, Overlord, and Supreme Ruler, score 1 VP per 5 Materials/Population, and score Goods separately at 1 VP each. Runtime resource aliases are canonicalized before Bot resource scoring.
...
- Bot Acquire and market Break through choose the eligible market card with the highest bot VP value after capping positive printed VP at 10 and adding 1 VP per market resource token, reading tokens from both legacy Market-resource maps and structured Market slot markers without double-counting synced copies, then the most total resource tokens on it, then the lowest-numbered slot. The gained card goes to the top of the Bot deck and market resource tokens enter the Bot resource pool. Market-Unrest tucks mirror into both legacy Market-Unrest maps and structured Market slot attachments. When Bot Acquire takes tucked Unrest, reading either representation, the acquired card is gained first and the Unrest card second, so the Unrest ends above the acquired card on top of the Bot deck. Market Break through returns tucked Unrest to the Unrest pile instead. Cultist-style cleanup that exiles the most-tokened Market card uses the same legacy-or-structured Market-resource token count.
...
- The Martian Bot Swap table effect can exchange a matching in-play Bot card with a matching Market card using the same Bot Market valuation as Acquire/Break through. The swapped-in Bot card receives Market resource markers from either the legacy Market-resource map or structured Market slot state, tucked Unrest is not taken by the Bot, and the gained Market card goes to the Bot discard as directed by the Bot table row.
...
- Bot effect rows can encode table-driven if-unable fallbacks for effects such as return-from-discard, recall, acquire, break-through, and top-card discard; the fallback resolves only when the primary effect cannot do anything, and import validation rejects if-unable branches on Bot effects whose runtime path cannot consult them.
- Practice mode is a simplified solo variant and should be feature-flagged separately from full solo.
- Practice setup follows the special practice variant: after Market setup, Exile the top 15 Main deck cards, then track a 12-turn/12-Progress market-churn clock. Each cleanup places one of those Progress tokens on a player-chosen Market card, mirrors that token into structured Market slot resource markers, offers the optional Market Exile choice immediately afterward for any tokenless Market cards, then continues the remaining cleanup steps. If every Market card has a token, no Practice Exile choice opens. When the 12-turn clock expires, complete that turn's cleanup and round-end Solstice, then score without the normal extra final round.
...
- Lowered aggression, quick setup, precious cards, short game, practice mode, solo difficulty levels, and campaign mode metadata are configuration options. Supreme Ruler campaign mode normalizes the Bot difficulty to Supreme Ruler. Commons cards tagged `supreme_ruler_campaign_extra` are excluded from ordinary solo setup and included only for Supreme Ruler campaign setup, allowing private data to model the extra campaign Commons cards without exposing card text. Campaign progress tracks wins, losses, defeated Bot nations, standard difficulty advancement, starting-deck add/remove choices, Supreme Ruler set-aside Commons cards, four-loss/five-win completion, and the doubled starting-resource bonus after a loss. Setup applies recorded starting-deck additions/removals before shuffling and drawing the opening hand. The doubled resource setup is 6 Materials, 4 Population, and 2 Progress, or 2 Goods instead of Progress with Trade Routes. Solo campaign gameover records a public-safe campaign outcome snapshot from normal scoring or solo Collapse, including win/loss, Bot nation, difficulty, human score, score kind, Bot score when relevant, and whether a campaign choice is required before updating progress. The app end-game summary can apply the campaign outcome plus the selected reward/return choice into the next deterministic campaign progress object used by setup, then exposes a versioned campaign-sheet export payload containing only public-safe IDs and progress metadata. Campaign-specific exception tags are accepted in private ruleset metadata.
...
- In the short game solo Bot setup, remove the physical bottom card of the Bot Dynasty deck, then move the current top Dynasty card to the Bot discard pile. Nation-specific Bot short-game setup exceptions can add further movements after this default sequence.
