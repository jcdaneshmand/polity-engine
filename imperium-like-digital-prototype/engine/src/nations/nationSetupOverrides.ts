  ...
  for (const ov of ruleset.setupOverrides ?? []) {
    ...
    if (ov.op === "create_side_area") {
      ...
      sideAreas[ov.areaId] = {
        ...
        cardIds: player.sideAreas[ov.areaId],
        visibility: ov.public === true ? "public" : "private"
      };
    }
  }
  return sideAreas;
...
export function applySetupActionTokenOverrides(player: PlayerState, ruleset: NationRuleset): void {
  for (const ov of ruleset.setupOverrides ?? []) {
    if (ov.op === "set_action_tokens_base") {
      player.actionTokensBase = ov.count;
      player.actionsRemaining = ov.count;
      player.actionTokensAvailable = ov.count;
    }
  }
}
