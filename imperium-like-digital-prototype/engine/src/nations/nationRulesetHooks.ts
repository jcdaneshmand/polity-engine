...
function evaluateCondition(G: GameState, playerId: string, condition?: EffectCondition): boolean {
  ...
  if (condition.op === "zone_empty") return getZoneCards(G, p, condition.zoneId).length === 0;
  if (condition.op === "zone_has_at_least") return getZoneCards(G, p, condition.zoneId).length >= condition.count;
  if (condition.op === "card_in_zone") return getZoneCards(G, p, condition.zoneId).includes(condition.cardId);
...
function getZoneCards(G: GameState, p: GameState["players"][string], zoneId: string): string[] {
  const direct = (p as any)[zoneId];
  if (Array.isArray(direct)) return direct;
  if (p.sideAreas?.[zoneId]) return p.sideAreas[zoneId];
  if (G.specialZones?.["0"]?.[zoneId]?.cardIds) return G.specialZones["0"][zoneId].cardIds;
