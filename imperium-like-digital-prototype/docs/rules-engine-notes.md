...
1. If the player has cards in their Nation deck, take the next/top Nation card and put it in the discard pile. If this was the accession/state-changing card, flip or change the State card at the correct point. Move an available Action token from the State card onto the Nation deck for that reshuffle marker, or into the Development area if that addition empties the Nation deck.
