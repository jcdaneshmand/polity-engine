...
- `npm run ops:rehearsal` now emits a public-safe production playtest checklist and verifies the runbook, deployment notes, hosted handoff, and npm scripts still cover local proof, Render build proof, commit-pinned hosted smoke, hosted browser QA, admin cleanup, support intake, storage expectations, and the private-data boundary.
...
Latest operations note: Track A now has an executable public-safe rehearsal gate. `npm.cmd run ops:rehearsal` checks that the runbook, deployment notes, hosted release handoff, and npm scripts still document the required pre-invite proof chain, admin cleanup rehearsal, support intake, persistent storage and backup expectations, and private-data boundary. `npm.cmd run verify:production-local` runs `render:verify` plus this rehearsal; live release proof still requires commit-pinned `smoke:hosted` and `qa:hosted-browser` against the deployed origin. Verification passed on 2026-08-10: `npm.cmd run test:local-qa-scripts`, `npm.cmd run ops:rehearsal`, `npm.cmd run verify:production-local`, live `npm.cmd run smoke:hosted` against `https://polity-engine.onrender.com` reporting live commit `59304a45acf7c83b96c51e109eaac78b07fa5cfc`, and live `npm.cmd run qa:hosted-browser`.

Latest custom Commons note: Track B now supports explicit custom Commons composition with public-safe card IDs. `GameOptions.customCommonsCardIds` is normalized, `buildCommonsSetup` intersects requested custom IDs with the normal Commons setup filters, and setup reports requested/missing/rejected custom IDs. The New Game setup screen exposes a Custom Commons picker after confirmed local data provides `commons_set_id=custom` cards, with QA-readable `data-qa="custom-commons-setup"` metadata. Browser QA now applies a fictional local CSV bundle, switches to Custom, and verifies the picker sees/selects the uploaded custom Commons cards.
